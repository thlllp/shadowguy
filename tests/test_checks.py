...
def test_resolve_check_critical_success_requires_margin_at_least_critical_margin():
    # difficulty 9 -> pool_for_difficulty 0, so only the attacker rolls: every one of
    # its CRITICAL_MARGIN dice hits and the margin is the pool size itself.
    roll = resolve_check(stat_value=CRITICAL_MARGIN, difficulty=9, advantage=0, rng=AlwaysSix())
...
def test_resolve_check_critical_failure_requires_margin_at_most_negative_critical_margin():
    # The mirror image: stat_value 0 means only the opposition rolls, so its every-die
    # hit is the whole margin, negative.
    difficulty = 9 + 2 * CRITICAL_MARGIN  # pool_for_difficulty -> CRITICAL_MARGIN dice
    roll = resolve_check(stat_value=0, difficulty=difficulty, advantage=0, rng=AlwaysSix())
    assert roll.margin == -CRITICAL_MARGIN
