...
**Known economy inversion:** jobs pay ~82–119 cash per stamina against a best gig of ~183–216 (simulated over the three presets, playing each activity's best-odds choice), so a player optimising for cash should ignore the fixers entirely. This predates the typed stages (the original lone gig already beat the original 2-stage jobs per stamina) and is only partly offset by jobs being the sole source of Rep and faction standing. The levers are `REWARD_BASE`, the gig payouts in `content.py`, and `Scene.stamina_cost`. Nothing has been retuned for it yet — don't add content that assumes jobs are the lucrative option until it is.
