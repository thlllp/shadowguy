...
**Runners & crew (`runners.py`, `app.BarScreen`, `Character.crew`).** The `RIVAL_RUNNERS` roster — Specter (Netrunner), Juncture (Solo), Mireille (Infiltrator) — are the hireable operators; each carries a `rating` (their effective `skill_value` at their specialty, for the run-time crew effect once that lands) and a `hire_cost`. A `BAR` location opens `app.BarScreen` (routed like the shops), where you pay a runner's fee to add their id to `Character.crew`; ContactsScreen's Runners panel marks who's on the crew. Their `archetype` string is exactly a role's `specialist`, so a hired runner slots straight onto the role it fits. **This is the recruiting counter only** — a crew member does nothing on a job yet; assigning them to a `Role.filled_by` (respecting the many-on-site / one-remote cap above) and the run-time effect (a remote Netrunner cracks the objective from the car; on-site muscle joins the fight) are the next increments. First-slice simplification: the whole roster is hireable at *any* bar; seating runners at particular bars the way `fixer.create_fixers` seats fixers to territories is the natural follow-up.
...
src/shadowguy/
  ...
  runners.py      hireable runner roster (specialist/rating/hire_cost); recruited onto Character.crew at a BarScreen
