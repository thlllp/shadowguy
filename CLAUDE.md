...
**Every difficulty constant in the game is still written on the old d20-era DC scale** (~9–21) and converted at the one chokepoint by `pool_for_difficulty` (`round((DC-9)/2)`, floored at 0), so the migration didn't have to re-derive them by hand. Two things to know before tuning against a DC:

- **The conversion is coarse and lopsided.** DCs 9–21 squash into opposing pools of **0–6**, while player pools run 2–15. `DIFFICULTY_BASE[0]` (DC 10) converts to **0 opposing dice** — a tier-0 job approach cannot fail unless you whiff every die. Two DCs one apart often convert to the same pool, so tuning a constant by ±1 frequently does nothing at all.
- **Critical failure no longer scales the way a natural 1 did.** It was a flat 5% tax on every build; it's now a function of how outmatched you are (~0% at pool 8 vs DC 13, ~11% at pool 2 vs DC 19). Since a critical failure is what routes a job stage into a fight, weak builds go loud more often than the old rule implied.
...
**A generated job offers a subset of each pool, not the whole thing.** `generate_job` takes the full pool `FULL_POOL_CHANCE` of the time and exactly `PARTIAL_POOL_SIZE` of it otherwise (about a 35/65 split today), keeping pool order so the clean approach still reads before the bloody one. (The one exception is a specialist job — see below — whose lead is pinned through the draw.)

**Some jobs are a specialist's work** (`Intrusion` today, the Netrunner's). `jobs.archetype_specialist()` is the test, and it is **derived, not tabulated**: a job every one of whose beats leads with the same `SPECIALIST_FOR_STAT` specialist *is* that specialist's contract, so there's no field beside the table to drift from the approaches actually in it. Two consequences. `generate_job` **pins the lead through the partial draw** for these — a generic job may withhold the very approach you specced into (that's the point of the draw), but a Netrunner job that offers no netrunning would be a contradiction, so the lead survives and only the rest is sampled. And `Scene.roles` falls out already-correct: an Intrusion reads as Netrunner on every beat, its `hack` objective the one `REMOTE` role, which is what tells the player on `FixerOffersScreen` that it's their kind of work — no label needed.

The stage-level cross-stat rule below still holds for them: a specialist job **guarantees its specialist a lane, it does not lock anyone else out.** An Enforcer can take an Intrusion and bleed through it (~15% completion vs the Hacker's ~69%) — bad work for them, not a closed door. That's the difference between a specialist job and a gated one, and it's why these obey the same import guard as everything else.

Why it exists is balance, not flavor. The generic table (Heist/Extraction/Sabotage) is Strength/Agility work — measured over 3000 jobs it offered `intelligence` on **30.7%** of stages and `perception` on **9.0%**, against ~52% each for strength/agility. So a Hacker failed most stages at pool 2–3 vs DC 13–16, finished **11%** of jobs, and rationally never left the gig treadmill (gigs 117 cash/stamina vs jobs 29, at a fifth of the death rate). Adding one Netrunner archetype moved the Hacker to **~69% completion, 220 cash/stamina, 3.3% deaths** — a ~1.9x job-over-gig premium, the same ratio the Enforcer already had. **`perception` is still at ~9% and still has no job that wants it** — it's reachable only via the optional complication stage and legwork, so a Perception build remains locked out of jobs. A Recon archetype is the obvious next one. So two Heists are not the same Heist: one leaves the door open for your build, the next withholds the very approach you specced into, and *which ways in a job happens to have* is part of what makes one fixer's offer better than another's. This is why pools want to stay wider than `PARTIAL_POOL_SIZE` — a pool of exactly two never varies, and one smaller than two would make `rng.sample` raise mid-generation (guarded at import). Note `PARTIAL_POOL_SIZE` is an **exact draw size, not a floor**: widening a pool adds approaches the *full-pool* roll can reach, but the partial draw stays two. If you want a wider pool to also mean a more variable partial draw, that's a change to `generate_job`, not just to the table.
...
Selecting one of these locations from the MainMenu **Local** tab pushes a `ShopScreen` (`app.py`) instead of being a no-op. `shops.CATALOG` maps each shop `LocationKind` to a fixed list of `Item`s (id, name, price, `stat`, `bonus`). Buying spends `Cash` and appends an `shops.InventoryItem(item_id, equipped=True)` to `Character.inventory` — a `list[InventoryItem]`, duplicates allowed, so the same item can be bought (and owned) more than once. Items are **persistent, not consumable**, and **only an equipped item counts** (`shops.equipped_bonus` skips `equipped=False`): `Character.stat()` adds up every *equipped* item's bonus for the requested stat on top of the raw attribute, so gear silently strengthens every check that uses that stat — jobs and legwork included, since both go through the same `stat()` call.
