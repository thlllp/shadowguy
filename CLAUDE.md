...
- **Corp mode** — 4X scale. Player controls a corp, area-control/resource game against rivals. Take over one of the 5 seeded Factions, collect territory income (net of `TERRITORY_UPKEEP` per district and of `logistics_strain`, the escalating bill for holding more districts than the corp's Development supports) and research, spend the day's two action points (`CorpState.action_points`, 1 per move — expand onto neutral ground, train employees at your Academy, upgrade your Research Facility, garrison a district, or attack a rival's), and spend research points on `corp_turn.TECHNOLOGIES`. Corps fight each other over territory — operatives vs. garrison + Security, see Corp conflict in `DESIGN.md` — and a corp that loses its last district ends the run.
...
| 66 | `CorpState.tasking_operatives` — operatives out on `tail_runner`/`gather_intel`/`sabotage`, tracked separately from the capped operatives pool (`operative_max`) so a corp at cap can still dispatch them; they return to the pool the next day tick via `return_tasking_operatives`, clipped at the cap same as `advance_training` |
| 68 | `corpmap.Territory.is_slum` — the `SLUM_COUNT` neutral districts of houseless encampments, which generate no locations and rest free (`lodging_cost`). A pre-v68 pickled `Territory` lacks the attribute outright: pickle restores a dataclass through `__dict__`, never `__init__`, so the `= False` default never applies and the first map render `AttributeError`s in `corpmap._label`. `claim_territory` clears the flag, since a corp moving in makes the ground non-neutral |
| 69 | `corpmap.Territory.is_outskirts` — the `OUTSKIRTS_COUNT` (5) neutral grid-edge districts: free lodging and an `O` map tag like a slum, but they still generate locations. Exactly v68's pickle hazard again (a pre-v69 `Territory` lacks the attribute, so `corpmap._label` `AttributeError`s on the first render); `claim_territory` clears it. Landed alongside the board going `TERRITORY_COUNT` 65 → 260 on a 14×24 grid — no save-shape change from the size itself, but a pre-v69 map is a quarter the districts |
| 70 | a fifth `factions.Faction` — Sanctuary Holdings, on the new `FactionSpecialty.FAITH`. No field changed shape, but a board is laid out once per run against the `FACTIONS` list of the moment: a pre-v70 map has no Sanctuary bloc, and its `CorpMap.relations` (seeded from `relations.ENTITY_IDS` at generation) holds no `faction_sanctuary` pair at all — `relations.relation` is a bare dict index, so any lookup reaching that id `KeyError`s |
| 71 | `matrix.Ice.human` — a spider (human defender) tag `Program.detect`'s `_guardian_tag` reads. A pre-v71 `Ice` is embedded directly (not by id) into a pickled accepted matrix job (`MatrixNode.ice`/`MatrixNetwork`/`scene.MatrixStage`/`Character.accepted_jobs`) and restores via `__dict__`, so it lacks `.human` outright — `_guardian_tag` would `AttributeError` the first time Detect is installed and a guarded node is viewed |
| 72 | `corpmap.CorpMap.faction_grudge` — a per-faction tally of jobs that landed negative standing on it (`scene.apply_outcome`), read and reset by `rivals.resolve_rival_day` when a faction bribes a gang against the runner (`rivals._pick_bribe_gang`, `RivalAction.bribe`). `CorpMap` is embedded directly (not by id) in the save bundle, so a pre-v72 map lacks the attribute outright — the first `apply_outcome` call with a negative `standing_delta` would `AttributeError` on `corp_map.faction_grudge.get(...)` |
| 73 | `Character.owned_apps` — ids from `shops.APPS_BY_ID`, the Phone's new App Store (`screens/info_screens.py`'s `AppStoreScreen`). Mirrors `owned_programs`' shape, a flat set of owned ids, so a pre-v73 `Character` lacks the attribute outright — the first `shops.owned_app_bonus` call (travel, `ShadowguyApp.rest_cost`, or a gang toll) would `AttributeError` on `character.owned_apps` |
