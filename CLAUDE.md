...
**The turn model** is move up to `speed` tiles, then one action — `speed` is a field on the unit, so a future ability can raise it rather than a global constant. Enemies A*-approach to melee then attack (ranged enemies are a later `Enemy` attribute, not a special case here). **Fleeing is positional and always works**: reach an exit tile and leave, no roll and no parting shot (the risk was crossing the room to get there). Same law as `combat.py`'s flee — *a fight must never be a cage* — enforced spatially instead of by a Dodge check.
...
**Balance — far swingier than abstract combat, by design.** Position and loadout dominate. Simulated with a simple auto-player, a tier-2 Hacker dies **~64% rushing in with a monoblade** but **~8% kiting with a pistol** (abstract-combat baseline 25%): a competent/ranged runner is *safer* on the grid, while a melee-only light build that crit-fails into a high-tier fight is in real danger. This is **left as-is on purpose** — fleeing to an exit is always available, and the 64% is a deliberately naive rush, not typical play. But it means the flee/exit rules, `TACTICAL_FIGHT_CHANCE`, and enemy counts are not places to tune casually, and the auto-player's *policy* moves these numbers as much as the constants do. Re-run the sim (presets × tiers, a reasonable policy) before touching enemy count, `cover`, or the flee rules.
...
src/shadowguy/
  ...
  app.py          Textual App: CharacterCreationScreen (start) + MainMenu + FixerListScreen/FixerOffersScreen + SceneScreen + CombatScreen + TacticalScreen + CorpMapScreen + ShopScreen + CorpHQScreen + InventoryScreen/SkillsScreen + QuitMenu/LoadMenu
```
