...
All three share the same `Scene`/`Stage`/`Choice`/`Outcome` data model, distinguished by `Scene.kind`. A stage of any of them can be a **fight** instead of a set of choices (`Stage.combat`; see Combat below) — that's a property of the stage, not of the `SceneKind`.
...
- **Gig** — small single-stage activity for quick resources, no Fixer needed. Hand-authored in `content.py` and wired up through `app.STATIC_ACTIVITIES`; they're where the skills jobs and legwork don't reach earn their keep (the chem trial is the Body block, the ring fight the weapon skills, the card table and the mark the social read). Because a gig is optional and the player self-selects into it, a gig **may** theme its choices on one stat — unlike a job stage, which may not. Gigs are also the only place an `Outcome` charges cash (a lost bet at `GIG_CARD_TABLE`), and **cash is not floored the way health is** — `apply_outcome` subtracts the loss straight off `Character.cash`. What keeps that from going negative is a gate, not a clamp: `Scene.max_cash_loss` is the worst `cash_delta` any path through the scene can charge (derived from the outcomes, never written beside them), and `MainMenu` refuses a gig the runner can't cover, labelling it `can't cover the stake`. You may not sit down at a table you can't pay out on. Flooring instead was the bug: at `cash` 0 every losing outcome costs nothing, so a runner who spends down at a shop — gear is a permanent stat bonus, so there's always something to buy — farms the card table with its entire loss column disabled. The gate lives on the gig path in `MainMenu.on_list_view_selected`; if a job or legwork scene ever charges cash, it needs the same gate, because nothing downstream will catch it. `rep` *is* floored at 0 (like health, unlike cash): a botched gig can burn rep you earned, but 0 is being a nobody and there's nothing below it. Cash is the one resource whose floor is a gate rather than a clamp, because cash is the one you can be made to *pay*.

  **A gig also only shows up where it makes sense.** `app.GIG_LOCATION_KINDS` maps each gig's id to the `corpmap.LocationKind`(s) it's plausible at (a ripperdoc's chem test reads as a clinic *or* a pharmacy, so it lists both; the card table and the rooftop-bar mark are `SOCIAL`-only because nothing else fits). `MainMenu._refresh` only lists a gig while the runner's current territory has a `Location` of one of those kinds among its `LOCATIONS_PER_TERRITORY` — same filter test as job-site gating below, but keyed on location *kind* present locally rather than on a specific `target_territory_id`, since a gig has no single fixed site. A gig with no listed kind is a startup error (checked against `STATIC_ACTIVITIES` at import), so a new gig can't silently stay hidden everywhere for want of an entry.
...
The load-bearing rule, enforced at import over the whole pool (so it holds for every subset that can be drawn): **the approaches in one stage must sit on different core stats.** A job stage is a gate every build has to pass, so two approaches on one stat hand that stat's runner a second bite and everyone else nothing — the same failure `corpmap._filler_pool` guards against on legwork. (Gigs are exempt: they're optional and self-selected, so a gig may be themed on one stat — `content.GIG_CHEM_TRIAL` is deliberately two Body checks.) A specialist is therefore *meant* to have stages that don't suit them; they bleed through on the easy approach rather than being locked out. Simulated over the three presets × three archetypes, that lands at 45–61% completion with under 2.2% deaths and 3–6 health spent — because a **failed stage still advances** (`failure` carries `next_stage`), so failure costs health and the final stage's reward, never the run.

**Gig payouts were rebalanced down** so a plain success lands ~75–100 cash (crit success ~130–165), scaled down from the original 180–400/300–650 range, and every failure/crit-failure cash penalty was scaled down with it so the card table's stakes stay proportionate. Since every gig defaults to `Scene.stamina_cost` 1, that puts gigs at roughly 75–100 cash per stamina. This was **not** re-simulated against jobs (~82–119 cash per stamina per the older sim, below) — the old "jobs pay less than gigs" inversion this section used to describe no longer clearly holds, and may now have flipped the other way. Re-run the balance sim before relying on either number. The levers are `REWARD_BASE`, the gig payouts in `content.py`, and `Scene.stamina_cost`.
...
**A fight is a `Stage`.** `Stage.combat` holds an `Encounter` (prompt, enemies, and a `victory`/`escape` `Outcome`), and it's routed to by an ordinary `next_stage` like anything else — so nothing needed a new effect pipeline. A stage is *either* choices or a fight, never both (guarded in `Scene.__post_init__`): a combat stage's "choices" are `combat.available_actions`, which come from the runner's own gear and skills rather than from the scene.
...
src/shadowguy/
  ...
  combat.py       Enemy roster, rounds, the six-stat action set, Drop/CombatOutcome; imports no scene
  scene.py        Scene/Stage/Choice/Outcome/Encounter data model, resolve_choice(), apply_outcome()
  content.py      hand-authored gigs (wired via app.STATIC_ACTIVITIES) + example job/legwork data
  ...
  shops.py        retail LocationKinds: Item catalog (incl. weapon skill/damage), consumables, buy/sell/equip
  app.py          Textual App: CharacterCreationScreen (start) + MainMenu + FixerListScreen/FixerOffersScreen + SceneScreen + CombatScreen + CorpMapScreen + ShopScreen + InventoryScreen/SkillsScreen
```
