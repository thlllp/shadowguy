"""Retail LocationKinds: what exists, what it costs, and buying/selling it with Cash.

Item is persistent stat-boosting gear (bought once, equipped/stowed, never used
up). Consumable is single-use: buying it appends to Character.consumables. What
happens *after* a purchase — equip state, popping a used consumable, installing
a Program on a deck — lives in inventory.py instead; this module only ever adds
or removes a Character's inventory/consumables/owned_programs entries, gated on
cash and standing.

Making something rather than buying it — scavenging a Junkyard, fitting a Mod,
cooking a Consumable — is workshop.py, which imports the catalogs here and is
never imported back. This module's mod *catalog* (Mod/MOD_CATALOG/MODS_BY_ID,
the named WEAPON_MOD_SLOTS layouts and their stock parts) stays put: buy_item
seeds a bought gun's slots from it and effective_item folds installed mods into
the stats combat.py reads, neither of which is bench work.

These locations can land on neutral ground or in a corp district's
non-specialty slot (see corpmap_gen.FILLER_KINDS), so a job can target one —
...
def slot_usage(inventory: list[InventoryItem], slot: Slot) -> int:
    return sum(_slot_cost(item) for item in equipped_items(inventory) if item.slot is slot)


def fits_in_slot(inventory: list[InventoryItem], item: Item) -> bool:
