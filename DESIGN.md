...
**The turn model**: move up to `speed` tiles, then one action; `end_turn` then runs the ally phase, the enemy phase, and opens the next turn. **Enemy reach is per-enemy** (`combat.Enemy.reach`, tactical-only) and now *follows the weapon in its hands* — `weapon_range` of whatever it carries, the same call the player's reach goes through: armed guards shoot from `FIREARM_RANGE` 8, street muscle and the bruiser close to `MELEE_RANGE` 1. Guards previously shot from a hand-set 6; a gun that reached less far in NPC hands than in the player's was exactly the second source of truth the derived stat block removes. **Both sides run the same phase** (`_ai_phase(allied=...)` over `_advance_and_attack`): pick a target, close via A* until it's in reach, then hit it — a ranged unit holds distance because it stops the moment it has a shot, melee has to walk all the way in. **Fleeing is positional and always works** — reach an exit tile, no roll, no parting shot — the same law as `abstract_combat.py`'s flee, enforced spatially.
