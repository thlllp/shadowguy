...
**The run-time effect exists on the grid**: a hire fights beside you in a job's tactical stages (`crew_working`, `combat.crew_stats`, `_CREW_PROFILES` — see Tactical combat), where `rating` is finally read as their attack pool. Abstract fights and non-combat stages still don't field them. Going down on a job has consequences that outlive the fight: `Character.dead_runners`/`arrested_runners` (save v42), written by `resolve_downed_crew` and read by `runner_available` — a killed runner is off the roster for the run, an arrested one for `ARREST_DAYS`. **Still to come**: assigning a hire to a `Role.filled_by`, and crew in `combat.py`'s abstract fight. First-slice: the whole roster is hireable at *any* bar (minus whoever is dead or inside).
