...
**Grenades are the one action that's aimed at a tile, not a unit.** `throw_grenade` reuses `combat._throw`'s three `shops.EffectKind` branches (`combat_consumables`, promoted public for exactly this reuse), but `COMBAT_STUN`/`COMBAT_DAMAGE_ALL` land as a blast — every enemy within `GRENADE_RADIUS` (chebyshev distance 1, a literal 3x3 square) of a target tile, not everyone standing, since the grid gives a grenade something abstract combat doesn't have: a place to actually miss. `COMBAT_ESCAPE` (a smoke grenade) stays positionless, same as the abstract fight — walking out isn't aimed at anyone. Picking the tile is a small modal turn state (`TacticalState.aim_cursor`/`pending_grenade_index`, `begin_grenade_aim`/`move_aim_cursor`/`confirm_grenade_aim`/`cancel_grenade_aim`): arrow keys drive the cursor instead of the player while it's set, `legal_grenade_target` gates the throw on `GRENADE_RANGE` and `has_line_of_sight` (same shape as a weapon's range+LOS gate), and backing out costs nothing — only a resolved throw spends the turn's action or pops the consumable. First-slice tuning, like the grenade catalog itself (`shops.py`'s `_CONSUMABLE_ROWS` comment) — `GRENADE_RANGE`/`GRENADE_RADIUS` aren't yet balance-simulated.
