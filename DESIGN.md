...
**Standing gates which tier a doc will fit, and the gate runs opposite to price.** `Cyberware.min_standing` is the same field `shops.Item`/`Consumable`/`Program` each carry, and `RipperdocScreen` hides a row above it outright rather than showing it locked — exactly `ShopScreen`'s `if item.min_standing > standing: continue`. It's set per *tier* (`CYBERWARE_TIER_MIN_STANDING`: T1 0, T2 3, T3 1, T4 0), never per piece, because the tier table already says what a tier means: Tier 4 is the cut-price knockoff that costs more of you, so any back-alley grafter fits one to a stranger, while Tier 2's cleaner install is what a doc saves for regulars. **Tier 1 is open to everyone**, so no *effect* in the catalog is ever locked behind a relationship — standing buys a better trade-off on the same implant, never a bonus you couldn't otherwise get.
...
**Standing is floored at 0 for gating** (`_effective_standing`). Every row's `min_standing` is `>= 0`, so a *negative* standing — one failed gig at the clinic is enough, `GIG_FAIL_STANDING_HIT` — would otherwise hide the whole catalog rather than just the gated tiers, breaking "Tier 1 is open to everyone" outright. A doc you've annoyed stops cutting you deals on the good chrome; they still sell you the baseline. **`shops.py` deliberately does not do this** — a shopkeeper who dislikes you can refuse their entire stock, because no shop catalog commits to an always-available tier the way this one does.
