...
Threaded like `smartlink_bonus`: the two **player** attack sites (`abstract_combat._attack`, `tactical.player_attack`) pass it into `resolve_hit`'s `base_damage` rather than it living inside `resolve_hit`, because that function is the shared formula for both directions of a fight and `combat.Enemy` has no Strength to fold in. **So it is player-only** — a hired runner swinging a blade beside you resolves through the same `resolve_hit` with their `Enemy` stat block and gets nothing.

**This is a large, deliberate buff and is not balance-simulated.** Enemy health tops out at 11 (`ENEMY_TIERS`' Chromed Enforcer) against a 4–7 base damage melee weapon, so a Strength 6 runner one-shots the whole roster with a katana, and at the top end Strength alone out-damages every weapon in the catalog. It also makes `UNARMED` (base damage 0) a real attack rather than a fallback: bare hands at Strength 6 out-damage a pipe pistol. That's what Strength buys — it carries only two skills (see Stats and skills), and this is the compensation.

**The intended counterweight is on the enemy side, and isn't built yet.** `Enemy` collapses all mitigation into two hand-set scalars — `defense` (the difficulty your attack rolls against) and `toughness` (the soak pool) — where the player's same two numbers are *derived*: `player_defense` is `DEFENSE_BASE + skill_value("dodge")` and `player_soak` is `stat("body")` + equipped armor + installed cyberware. Enemies are meant to get the same treatment (a dodge, a body, armor of their own) rather than a flat number per roster entry, at which point the damage curve above is absorbed by mitigation that scales instead of by re-tuning the bonus. **Until that lands, read the one-swing numbers above as temporary, not as the intended end state** — and re-run the balance sim after it, not before.
...
**The turn model**: move up to `speed` tiles, then one action; `end_turn` then runs the ally phase, the enemy phase, and opens the next turn. **Enemy reach is per-enemy** (`combat.Enemy.reach`, tactical-only): armed guards shoot from reach 6, street muscle/the bruiser close to reach 1. **Both sides run the same phase** (`_ai_phase(allied=...)` over `_advance_and_attack`): pick a target, close via A* until it's in reach, then hit it — a ranged unit holds distance because it stops the moment it has a shot, melee has to walk all the way in. **Fleeing is positional and always works** — reach an exit tile, no roll, no parting shot — the same law as `abstract_combat.py`'s flee, enforced spatially.
