...
**The turn model**: move up to `speed` tiles, then one action; `end_turn` then runs the ally phase, the enemy phase, and opens the next turn. **`moves_left` only rations anything while `TacticalState.in_combat`** (`state.alarm` or any live enemy `alerted`) — an ordinary tactical fight is this from the first turn (`Unit.alerted` defaults to `True`), so nothing changes there. A burglary's guards start unalerted, and walking a quiet house before `check_detection`/`raise_alarm` catches you spends no `moves_left` at all (`legal_moves`/`move_player`/`take_stairs`/`attempt_lock` all gate the decrement on it) — only the fight that follows rations movement, matching the guard's own turn (`_ai_phase` skips an unalerted unit outright, so there was never anything to react to). The step that gets you spotted is itself free, since nothing was hunting you when you took it; `in_combat` flips (from `check_detection` inside the same call) before the *next* step, not the one just taken. One-way, same as `alarm` — nobody un-notices you. **Enemy reach is per-enemy** (`combat.Enemy.reach`, tactical-only) and now *follows the weapon in its hands* — `weapon_range` of whatever it carries, the same call the player's reach goes through: armed guards shoot from `FIREARM_RANGE` 8, street muscle and the bruiser close to `MELEE_RANGE` 1. Guards previously shot from a hand-set 6; a gun that reached less far in NPC hands than in the player's was exactly the second source of truth the derived stat block removes. **Both sides run the same phase** (`_ai_phase(allied=...)` over `_advance_and_attack`): pick a target, close via A* until it's in reach, then hit it — a ranged unit holds distance because it stops the moment it has a shot, melee has to walk all the way in. **Fleeing is positional and always works** — reach an exit tile, no roll, no parting shot — the same law as `abstract_combat.py`'s flee, enforced spatially.
