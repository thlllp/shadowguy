...
**Staffing is presence only, not a ladder.** `_make_gang_members` gives one `LocalCharacter` per `GANG_RANKS` tier (same helper `_make_officers` uses) but no rep/standing gate or dialogue. Soldier/lieutenant appear in `CorpMap.characters()` but never surface in Locals (nothing moves their `local_standing`). `GANG_DEN` *is* routed through `CorpMapScreen`'s map hover/selection boxes (`GangDenScreen`) — see Gang deliveries below — just not to a dialogue screen the way `CORP_HQ` is.

**Map/menu presence, plus two live consequences.** A territory carrying gang turf shows a `gang: <name>` suffix alongside `owner:` in `CorpMapScreen`, both in the map's own territory info and its hover boxes. Beyond display, gangs have standing, a turf-entry encounter (below), and a job of their own (Gang deliveries, below) — still no dialogue.
