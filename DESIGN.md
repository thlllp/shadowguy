...
**Two modules, split by who needs which half.** `combat.py` is what a fight is *made of* — the enemy roster (`Enemy`/`ENEMY_TIERS`/`roll_enemies`/`NPC_WEAPONS`), `crew_stats`, the hit formula (`resolve_hit`/`soak_damage`), `player_defense`/`player_soak`, the reach bands (`MELEE_RANGE`/`THROWN_RANGE`/`FIREARM_RANGE`/`weapon_range`, moved down from `tactical.py` once `Enemy.reach` needed them), `equipped_weapons`/`consumables_with`/`combat_consumables`, `attack_verbs`, `smartlink_bonus`, and `Drop`/`drop_for_result`. It runs no rounds. **All three fight surfaces import it** — `abstract_combat.py`, `tactical.py`, `matrix.py` — and none imports another, which is the layering that stops a grid attack and an abstract one from drifting into different formulas. `abstract_combat.py` is fight surface 1 on top of it: `CombatState`/`Fighter`, `Action`/`ActionKind`/`available_actions`, `start_combat`/`take_turn`, the six action handlers, `CombatOutcome`, and the round's tuning constants (`BRACE_*`, `READ_*`, `INTIMIDATE_DIFFICULTY_BONUS`, `FLEE_*`, `FREE_ROUND`). Only three screens need that half — `combat_screen.py` for the whole round loop, `scene_screen.py` and `corp_map_screen.py` for `CombatOutcome` alone. `Enemy` deliberately stays in `combat.py` rather than moving with the round loop — it's reachable from a pickled `scene.Encounter`, so moving it would invalidate every save for no gain.
...
## Tactical combat (`shadowguy/grid.py`, `shadowguy/tactical.py`, `scene.TacticalStage`, `screens/tactical_screen.py`)
...
**The other half of a crew.** An on-site hire is a body on the board with a `combat.Enemy` stat block; a support hire never appears on the map at all and instead does things to the *building* on the player's order (`tactical.Support`, `support_for`, `support_tasks`, `run_support_task`; the `h` key and `HackerPickScreen`).
...
**Every non-matrix job fight is tactical** — `{stage}_fight` always holds a `TacticalStage`. (A Data Heist's fights are `MatrixStage` instead; see Data Heist below.) Abstract `Encounter` combat (`CombatScreen`) still exists — a legwork critical failure and a gang-turf ambush both route into one — but a job's own fights no longer do.
