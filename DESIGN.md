...
**Support is a role, not a placement.** `runners.SUPPORT_ARCHETYPES` gates it — Netrunner today — and `Character.hire_for_job` refuses the posture to anyone else rather than selling a cut for an empty comm link. A solo standing across the street contributes nothing. **Riggers are the other half and don't exist yet**: when they do they're one archetype row and one entry in that table, not a new system — a drone on overwatch is the same "somebody else acts on their own turn" shape.
