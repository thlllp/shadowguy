...
Design rationale for Shadowguy's game systems: what each system does, why it
works that way, and which numbers are load-bearing.

**Read this before changing game behavior.** `CLAUDE.md` holds the coding
guidelines, the codebase map, the module-layering rules and the save-version
history; this file holds the "why" behind the systems those modules implement.
Much of what looks like an arbitrary constant here was chosen against a balance
simulation, and the sections say so where that's true.

## Stats and skills (`shadowguy/character.py`, `shadowguy/skills.py`)

Six **core stats** (`character.CORE_STATS`): `Body`, `Strength`, `Agility`, `Perception`, `Logic`, `Cool`. `Cash`, `Rep` and `Humanity` are resources, not checkable stats — `STAT_NAMES` is the union, and `Character.stat()` only folds gear bonuses into the six core ones. Health is a separate pool scaling off **raw** Body (`BASE_HEALTH + body * HEALTH_PER_BODY` = `10 + body * 5`), deliberately not `stat("body")`, so gear never moves max health.

**Nothing rolls a core stat directly.** Each stat carries skills (`skills.SKILLS`, **39** total); a `Choice` names a *skill*, and `skills.skill_value()` is what the dice see: the skill's tied stat (gear included) plus the rank invested in that specific skill. **Every weapon skill is an agility skill, one per weapon category** — `Pistols`, `Automatics`, `Longarms`, `Clubs`, `Blades`, `Archery`, `Throwing` and `Gunnery` all sit on `agility`, on the reading that what a weapon rolls is handling it, not the muscle behind it or the eye down the sight. They replace the old five (`Short Blade`/`Long Blade` merged into `Blades`, `Blunt` became `Clubs`, `Firearms` split three ways by how a gun is fired, `Misc Weapons` is gone and its tasers roll `Pistols`), so a build buys the guns it actually carries rather than "firearms" wholesale. `Gunnery` (turrets and stationary guns) is the one with nothing behind it yet — no mounted weapon exists anywhere in the game, so it is a placeholder for a system that isn't built.

That leaves the spread lopsided on purpose: agility **13**, logic **11**, cool 5, body/perception 4, strength **2** (`Grapple`, `Lift` — unarmed and hauling, which are not weapons). **A stat that carries few skills earns a direct mechanical bonus instead** — that's the principle behind Strength's melee damage (`combat.melee_damage_bonus`, see Combat), not a balance patch after the fact. Skill count is how *often* a stat gets rolled, so a two-skill stat is rolled rarely by construction; paying it back with a bonus that applies every time you swing is what keeps it worth buying without inventing filler skills to pad the table. Strength is the only stat that adds directly to damage, and agility is by far the widest: it already carried `Dodge` (the combat exit and the defense roll) and `Stealth`, and now carries every way to swing or shoot as well, so an agility build is both the hardest to hit and the best-armed. The categories cut the other way, though, and that's the point of splitting them — 13 agility skills is not 13 skills a fighter needs, it's a menu where a pistol build and a rifle build spend different points. **Nothing in the balance simulation has been re-run against any of this**, and the old sim figures below (which measured a strength/agility split, one `Firearms` skill, and a five-weapon catalog) predate it. Body stayed at four, net: `Resist Poison`/`Resist Disease` folded into a single `Fortitude` (both were "the body shrugging off an internal assault" wearing two names), `Lung Capacity` was cut outright, and a new `Running` skill took over the two job approaches (both literally about outrunning pursuit) that `Lung Capacity` used to cover. Perception is down one from its old five-skill base for the same reason as Body's `Fortitude` merge: `Read Face`/`Read the Room` folded into a single `Intuition` (both were "reading a person or a room" wearing two names). Nothing enforces five-per-stat, and cost is per-skill anyway, so extras and shortfalls just make a stat broader or narrower, not stronger or weaker. `Character.skill_ranks` is `dict[skill_id, int]`, fully populated (every skill starts at `STARTING_SKILL_RANK`). A skill id absent from `SKILLS_BY_ID` raises from `skills.skill_for()`, the single chokepoint — `Scene.__post_init__` runs it over every choice, so a typo fails when the scene is *built*, not mid-roll.
...
**Everything starts at 1** — all six stats, all 39 skill ranks — bought up from there. **A build's skill points have a third thing to buy: gear.** `convert_skill_point_to_gear` trades one of the 20 for `GEAR_EB_PER_POINT` (1000eb — roughly *one item*, since the catalog runs 80–1800) of creation-only budget, spent on `GearScreen` (`g`) and on nothing else. **It is never cash** — whatever is left when the run starts is written off (`discard_gear_budget`), which is what makes it a gear purchase rather than a way to print starting money, and the creation screen says so out loud rather than losing it silently. Only ungated gear is offered: standing is a relationship, and there is nobody to have one with before the run begins. **Deliberately uncapped** — a runner may pour all 20 points in — because the trade caps itself twice: ranks are the to-hit half of every fight (*weapons are the damage, skills are the hit*), so a runner who bought a katana and no Blades swings at `skill_value` 2 and misses nearly everything, and `SLOT_CAPACITY` limits what armor can be worn at once. Gear with no ranks behind it is a bad build, not a strong one. Entry point is `screens.menu_screens.TitleMenu` (New Game / Load Game / Test / Settings, pushed from `app.py`'s `on_mount`); New Game opens `ModeSelectScreen` (Runner / Corp — see Corp mode turn loop), and only the **Runner** branch reaches `CharacterCreationScreen`, spending `STARTING_STAT_POINTS` (6) and `STARTING_SKILL_POINTS` (20). A Corp game skips this screen entirely and never builds a runner. Runner asks one question of its own first (`BuildSelectScreen`): **Premade archetype** goes via `ArchetypeSelectScreen`, which applies a preset and *then* opens creation with both pools at zero; **Custom character** opens it with both pools full. The preset is a starting point rather than a separate one-way path — `r` on the creation screen resets it back to a blank build, and `escape` pops back to whichever screen pushed it. **The presets are never out of reach**: `restart_run` reopens `BuildSelectScreen` (not creation directly), and a save taken mid-build reloads as creation *on top of* it, so escape always has somewhere to go. Starting the run then goes through **`ShadowguyApp.begin_run()`** — both here and from `CorpSelectScreen` — which *clears* the setup screens rather than replacing only the top one: the map's own back is a no-op so they're unreachable either way, but a live `ArchetypeSelectScreen` one pop below home would `reset_build()` the character mid-run if anything ever over-popped. An unspent runner rolls `skill_value` 2 on everything — the build is entirely what those 26 points bought. (`TitleMenu`'s Test option opens `TestMenu`, a developer shortcut straight into a standalone tactical or matrix fight, reusing whatever `Character` `_new_run()` stamped out — reachable before `CharacterCreationScreen`. No job/gig involved; a tactical test fight resets health to max afterward.)

**Archetypes (`shadowguy/archetypes.py`) are the fast path**: Enforcer, Hacker, Infiltrator, Gunslinger, Street Samurai, Fixer — each a canned allocation of the same 6 + 20 points. **Gunslinger** is the ranged counterpart to the Enforcer now that weapons split by category (a rifle build wants `Longarms`, and nothing else buys one), **Fixer** is the only preset built on `cool`, whose `Leadership` is *read* rather than rolled and so discounts every crew hire for the whole run, and **Street Samurai** pairs Agility with Strength rather than Body — a rank-7 `Blades` duelist that answers a hit with `Dodge` instead of soak, `combat.melee_damage_bonus` doing for its Strength what it already does for the Enforcer's, topped off with a Deltaware Hydraulic Cyberarm installed straight into the preset (`Archetype.cyberware`, `Character.buy_creation_cyberware` — the first preset to spend its gear budget on chrome instead of a shop item, still capped to the ungated Deltaware/Trashware grades the same way `gear` is capped to ungated `Item`s). The originals moved with the table too — the Enforcer swings `Clubs` (and buys Strength for `melee_damage_bonus` as much as for the skills), the Infiltrator carries `Blades` because a blade is the weapon it can conceal, and the Hacker now spreads across all five skills a matrix fight reaches rather than one `Hack`, and puts every stat point on logic (its old 2 in perception bought nothing once `Pattern Seeking` gave way to the matrix skills). `Archetype.apply()` spends them through `spend_stat_point`/`spend_skill_point` rather than assigning fields, so a preset obeys the rank cap and cost curve exactly like a hand-built runner and **cannot buy anything the player couldn't**. **Each preset also ships a loadout** (`Archetype.gear`), bought the same way — its rank list deliberately leaves exactly the points its own kit costs unspent (two for most presets at today's rate, three for the Hacker since a top-tier cyberdeck is priced to cost a real rank rather than pocket change), `apply()` converts them and buys it — change a price and the rank list that ships it has to move too, which `_validate_preset` enforces, so a preset stays the *whole* build rather than dropping the player at a shop with a pool they may not know they have. `_validate_preset()` runs every preset against a fresh `Character` and requires it to spend both pools to exactly zero — deferred to first access of `ARCHETYPES`/`ARCHETYPES_BY_ID` (module `__getattr__`), not plain import, so importing `archetypes` alone doesn't construct a `Character`. Picking one (on `ArchetypeSelectScreen` — the presets are their own screen now, not a grid of cards on the creation screen) calls `reset_build()` first — a preset is the *whole* build, not a top-up. (`archetypes.Archetype` is a *character* preset, unrelated to `jobs.JobArchetype`, a job template.)

**Not every skill is rolled by something.** Coverage: 26 distinct skills across `jobs.ARCHETYPES`' approaches, 8 via `corpmap.LOCATION_SKILL` (legwork), the per-kind approaches in `gigs._GIG_TEMPLATES`, and combat (`abstract_combat.available_actions`: brace/read-the-fight/face-them-down/break-and-run, plus whatever skill each equipped weapon rolls). `Leadership` (`cool`) is *read* rather than rolled — scales crew recruiting terms (see Runners & crew). Gaps: `Clubs`, `Automatics`, `Archery` and `Throwing` are **weapon-gated** — no scene rolls them, so they're reached only by owning the weapon and swinging it (`Blades` is on two job stages, `Pistols` on Wetwork's objective, `Longarms` on the weapon-shop gig). `Gunnery` is reached by nothing at all: there is no mounted weapon to roll it. **Two of the six logic skills added with `Cybercombat`/`Computer` are still in that state** — `Medicine` and `Demolitions` are in the table with nothing rolling them yet, though each has an obvious hook already in the code waiting for it: `tactical.stabilize_ally`'s health kits (no check today), and grenade throws (also no check today). `Armorer` and `Chemistry`, the other two, are now rolled by the workshop's Mod installs and consumable crafting respectively (see Rest, lodging & property). `Sight` is reached only via legwork at a `WEAPON_SHOP`. The stat spread across approaches is agility 23, cool 19, logic 19, body 16, strength 16, perception 12 — the weapon approaches that had to leave the pools when weapons moved to agility (a stage's approaches must sit on three different core stats, guarded at import) became the strength skill their flavor already described, `Lift` forcing the vault/case/rack and `Grapple` putting down whoever's in the way, which is what keeps strength on 16. The presets buy narrow on purpose — a preset should read as an archetype, not a hedge — which is what makes each bleed on unsuited job stages. The Hacker is the deliberate exception, and only because the matrix stopped being one skill: covering `Cybercombat`/`Hack`/`Computer`/`Infer`/`Tinkering` at all is what costs it every point it used to spend outside its own arena.

**`_validate_preset` checks both halves agree**, not just that the pools hit zero: every stat a preset raises must carry at least one skill it buys. Nothing rolls a core stat directly, so a stat bought for skills the preset doesn't own is dead weight — which is exactly how the Hacker's perception and the Fixer's logic points first shipped.

**A preset's signature weapon is not free.** A runner starts with an empty inventory on `STARTING_CASH` (100eb), and only the Enforcer's `Clubs` has a weapon inside that budget (Pawned Knuckles, 80eb at a pawn shop). The Infiltrator's `Blades` wants a 400eb knife and the Gunslinger's `Longarms` a 650eb shotgun — which is why the Gunslinger buys `Pistols` 4 as well, to have something to roll over the first job or two. Bare hands (`UNARMED`, Grapple, off Strength) is the fallback until then.

**No preset is built on the remaining unrolled skills** (`Medicine`, `Demolitions`) — a preset that spent its 20 points on skills nothing rolls would be a trap, not an archetype. Whichever of them gets wired to a real check first is the one worth a preset. (`Armorer`/`Chemistry` are no longer in this group — see Rest, lodging & property — but still have no preset built on them; the workshop is a mid-run system, not something a fresh character walks in already good at.)
...
- The begin-gate can't deadlock: ranks 2–4 always cost 1, and 20 points can never push enough of 39 skills past rank 4 to exhaust the 1-point buys.
...
- **Gig** — small single-stage activity for quick resources, no Fixer needed. **Procedurally generated per-Location** by `gigs.generate_gig` from a per-`LocationKind` template. Attached to a place *and* a person — spawns at a `Location`, owned by one of its `LocalCharacter`s, rewards standing with them. Optional and self-selected, so it **may** theme its choices on one stat (unlike a job stage); each `LocationKind` template offers exactly two approaches and a gig presents both. Lives on `app.location_gigs: dict[location_id, Scene]`, not on `Location` (see Module layering in `CLAUDE.md`), refreshed by `gigs.refresh_gigs` on run-start and each day tick, consumed one-shot. **Spawning is a per-location roll, not a guaranteed fill**: each empty eligible location gets one `GIG_SPAWN_CHANCE` (0.3) shot per tick, so the map doesn't saturate with a gig everywhere on day one and a completed gig's slot trickles back rather than refilling the very next day. `CorpMapScreen` lists the current territory's gigs, gated only by being there.

  **The cash-stake gate exists but no current gig uses it.** `Scene.max_cash_loss` (the worst `cash_delta` any path can charge, derived from the outcomes) lets `CorpMapScreen` refuse a gig the runner can't cover — `apply_outcome` subtracts a loss straight off cash, and **cash is not floored the way health is** (flooring would let a broke runner ride every losing outcome for free). Health and rep *are* floored (rep at `REP_FLOOR`, see Stats and skills, above) — only cash is allowed to go unaffordable-and-refused instead. Today's gigs cost only health on failure so the gate never bites, but it's live the moment any scene charges cash — a job/legwork scene that charges cash needs the same gate.
...
Travel lives on `CorpMapScreen`: `*` is the **cursor** (`selected_id`, moved by arrows along connections, or clicking a node), `@` is the **runner** (`character.location_id`). `enter` moves the runner to the cursor's node, only if it borders `location_id`. Cost is **hours, not stamina** (`CorpMapScreen.action_travel`, see Time & the day clock): a hop costs `TRAVEL_HOURS_COST` (2.0), cut by an equipped `Slot.VEHICLE`'s `shops.Item.travel_reduction` (`inventory.equipped_travel_reduction`) — no daily allowance to run out of, travel never refused.

The Locals panel (`#map_local_boxes`, shown in map mode below `#territory_summary`) renders one Collapsible box per `Location` **in the territory the runner is standing in**, plus a Fixers box if any are present, phone-tile-style, at most one box open at a time — `CorpMapScreen.on_collapsible_expanded` collapses every other box in the same grid when one opens. A box's preview covers who works there (`Location.characters`), any gig offered there (`app.location_gigs`), other runners present (bar `Location`s only — the one place `rivals.py`'s DRINKING activity ever puts a rival runner), and a shop's stock (names only, capped at `_STOCK_PREVIEW_COUNT` before "+N more" — pricing lives on `ShopScreen`, not here; `WEAPON_SHOP` alone stocks 20+ items). An "Enter" row inside the box is what actually pushes the location's own screen: the five shop kinds → `ShopScreen`, `BAR` → `BarScreen`, `CORP_HQ` → `CorpHQScreen`, `HOSPITAL` → `HospitalScreen`, `JUNKYARD` → `JunkyardScreen`, `GANG_DEN` → `GangDenScreen`, `REAL_ESTATE` → `RealEstateScreen`, `APARTMENT`/`SAFEHOUSE` → `SafehouseScreen`. The rest are display-only. `CORP_HQ`/`GANG_DEN` are the two that need their owner threaded in as well as the `Location` (`FACTIONS_BY_ID[here.owner]` / `GANGS_BY_ID[here.gang_id]`).

**The panel does not follow the cursor.** Hovering or arrowing over another territory retargets `#territory_summary` — the bar directly above it — and nothing else. That split is what `_refresh_map_view` (map text + summary bar, called from `on_mouse_move`/`on_click`/`action_move`) vs. `_do_refresh_map` (both, called from travel/resume/post-action refreshes) enforces, and it is not just a display rule: a rebuild replaces those widgets outright, so refreshing the panel on mouse-move would snap shut whichever box the player had open. It follows that every box in the grid is somewhere the runner can act — so `on_collapsible_expanded` needs no travel gate (it previously re-collapsed a distant box and notified "Travel to `<name>` first"), and the "Enter" row resolves its `Location` against `character.location_id` rather than the cursor, which would find nothing. The grid always gets a Rest box (`_refresh_map_local_boxes`, same "rest" item id `#activities`' own Rest row uses). The grid sits in a bounded, independently-scrolling `height: 14` container (`#map_local_boxes_scroll`) rather than `1fr` — it's a strip below the map, not the main event, and an expanded box's content (a long stock list) can still run taller than that and needs to scroll internally instead of pushing everything else off-screen. Rebuilds are serialized through a single background task (`CorpMapScreen._map_locals_task`) — concurrent `Grid.remove_children`/`mount_all` calls against the same grid previously caused native crashes; a rebuild requested while one is in flight (`_map_locals_pending_id`) is picked up by that same task once it's free, rather than being dropped. Hover no longer triggers rebuilds at all, so this now guards rapid travel rather than mouse movement, and the pending value stays a territory id so a rebuild queued behind a travel resolves to the post-travel territory.

**Position gates jobs.** A job — and its legwork — can only be run while standing in `target_territory_id` (`CorpMapScreen._on_site`). Off-site the row reads `travel to <district>` and is a no-op — enforced in `on_list_view_selected`, not just the label. This stacks with timing (on-site but wrong day is still blocked; the label reports whichever gate bites first). Gigs are gated differently — to wherever a matching `LocationKind` is local, not one territory.

## Rest, lodging & property (`shadowguy/corpmap.py`, `shadowguy/shops.py`, `screens/corp_map_screen.py`, `screens/shop_screens.py` — `HospitalScreen`/`RealEstateScreen`/`SafehouseScreen`)

Lodging is charged by `ShadowguyApp.rest()`, not by the midnight tick. The runner can Rest wherever they are, whenever they want — it's an ordinary menu action (`CorpMapScreen`/`CorpScreen`), not something that fires as a side effect of some other action crossing midnight.

- **Lodging** is `LODGING_COST_PER_DEVELOPMENT` (5) × the district's Development — free where `corpmap.has_home(territory)` (an owned `APARTMENT`/`SAFEHOUSE`) or under an active security contract there. **Resting must never be blocked**: `rest()` pays `min(cost, cash)` and moves on. `ShadowguyApp.rest_cost()` is the one place the formula lives — both the charge and the menu-item preview (`CorpMapScreen`/`CorpScreen`) call it.
...
- **`rest()` itself heals a flat one point**, unconditionally — wherever the runner rests, whatever else happened that day (no Health Kit or location gate, unlike the cash charge above). `adjust_health` still clamps at `max_health`, so a full-health Rest is a no-op point. It also clears `Character.stun` outright via `mark_rested()` (see Combat).
- **How long a Rest runs is the Alarm Clock's business, not `rest()`'s** — `_rest_hours()` reads `Character.alarm_hour` (see The Phone), so a set alarm can make a Rest much shorter (or, set to the current hour, a full day) than `REST_HOURS_COST`. The heal, the fatigue halve and the lodging charge don't scale with the duration — a one-hour alarm nap pays and heals exactly what an eight-hour one does. First-slice, deliberately not yet a per-hour curve.
...
- **Workshop**: `Location.workshop_built` (`None` on every non-`PLAYER_OWNED_KINDS` location, so it follows `research_tier`'s "only meaningful on the right kind" convention). The apartment is injected with one already built; a safehouse starts without one — `SafehouseScreen` builds it for a flat `WORKSHOP_BUILD_COST` (400) in eb, same shape as `RealEstateScreen`'s purchase flow. Once built, `SafehouseScreen` offers two workshop actions, both spending `WORKSHOP_HOURS_COST` (2) win or miss, same as a Junkyard trip:
  - **Install/remove a Mod** (`shops.MOD_CATALOG`/`MODS_BY_ID`) on an owned weapon or wearable: `install_mod` charges the mod's cash price plus `SCAVENGE_MATERIALS` (see Junkyards) and rolls Armorer at `WORKSHOP_ARMORER_DIFFICULTY` (11) — a miss spends nothing, same "costs nothing but the trip" shape as `scavenge`. A mod attaches to one specific `InventoryItem` (`InventoryItem.mods`, capped at `MOD_SLOTS_PER_ITEM` = 2 per item), not to the catalog `Item` — a weapon mod carries `damage`, a wearable mod carries `defense`, enforced at import (a mod can't carry both). `shops.effective_item` folds an entry's mod deltas into the `Item` it resolves to; `equipped_items` and `combat.equipped_weapons` both go through it, so nothing downstream needs to know mods exist. `remove_mod` is free either way, same as uninstalling a deck program.
  - **Craft a Consumable** (`shops.CRAFT_RECIPES`) from `SCAVENGE_MATERIALS` plus `WORKSHOP_CRAFT_PRICE_FRACTION` (0.5) of its retail price, rolling Chemistry at `WORKSHOP_CHEMISTRY_DIFFICULTY` (11). Deliberately narrowed to the chem-flavored `TEMP_STAT_BOOST` consumables (`chem_x`/`chem_y`/`chem_x2`) — health kits and grenades stay `Medicine`'s and `Demolitions`' own hooks (see Stats and skills), not this one.

  This is what finally rolls `Armorer` and `Chemistry` — see the unrolled-skills gap in Stats and skills, now down to `Medicine`/`Demolitions`.
...
- **A rest only halves it.** `Character.mark_rested()` — called by both `rest()` and a hospital stay — does `fatigue //= 2` (and resets `last_rest_hour`) instead of clearing it outright — one good night's sleep takes the edge off a bad stretch, it doesn't erase it. A deep burnout costs several rests in a row to fully work off.
...
**Running a job (or its legwork) must not expire itself.** `CorpMapScreen` calls `spend_time(scene.hours_cost, protect_job_id=job.scene.id)` *before* pushing `SceneScreen` — a `scheduled_day` job run late on its one legal day (8–12h cost) could otherwise get pruned by its own time cost before the scene starts. `protect_job_id` exempts that one job (matched by `Scene.id`) from this tick's expiry check only — it's still removed normally once its scene resolves.

**An explicit "Rest" action remains** (`CorpMapScreen`/`CorpScreen`), spending `REST_HOURS_COST` (8) hours — just another `spend_time` call, so it can still cross a midnight boundary (firing every *other* day-tick effect above) but doesn't have to. It heals a flat point and halves fatigue — see Rest, lodging & property and Fatigue above. **The Phone's Alarm Clock overrides the flat 8** (`ShadowguyApp._rest_hours`, see The Phone below): with `Character.alarm_hour` set, a Rest runs only until that hour, wrapping to the next day and never resolving to 0. `ShadowguyApp.rest_label()` previews both the lodging charge and the actual duration on the menu item before the player commits.

**Vehicles cut travel time by a percentage, not a daily allowance.** `shops.Item.travel_reduction` (`Slot.VEHICLE` only): `inventory.equipped_travel_reduction` applies fresh to `TRAVEL_HOURS_COST` (2.0) each hop. Catalog: Beater Bike -10%, Tuned Coupe -20%, Armored Towncar -25%. `corp_map_screen._travel_hours(character)` is the one place the formula lives — both the charge and the pre-travel hint call it.
...
Jobs are gated behind a persistent Fixer roster (`fixer.FIXER_ROSTER` / `create_fixers()`). Each `Fixer` holds up to `max_offers` `JobOffer`s, procedurally generated (`jobs.generate_job`) from `jobs.ARCHETYPES` — **nine** templates (Heist / Extraction / Sabotage / Intrusion / Wetwork / Burglary / Data Heist / Bodyguard / Recon — Burglary and Data Heist documented in their own sections, the specialists and Bodyguard's odd shape below), difficulty/reward scaled by a day-derived tier, flavor from word banks. (Gang deliveries are a tenth kind of job but deliberately *not* an archetype — see Gang deliveries.)
...
Neither term costs anything upfront. `Character.crew` is `list[CrewHire(runner_id, job_id | None)]` (`None` = indefinite); one live hire per runner. For-job hires are discharged when their job leaves `accepted_jobs` (`_discharge_orphan_crew`). Crew earns `crew_experience` from completed jobs (see Post-creation experience) but no spend path yet.

**A for-job hire also picks a `CrewHire.on_site`** (`True`/`False` — on-site muscle or remote support), gated by that job's roster cap: `jobs.JobArchetype.max_on_site`/`max_support`, carried onto the generated `Scene` and read by `Character.job_roster_has_room`/`hire_for_job`. `None` (every archetype but three) is uncapped, today's original behavior. `max_on_site` counts the player, so Burglary's `1` leaves no on-site hire slot at all (a support hire only), Data Heist's `1`/`0` is solo end to end, Wetwork's `3`/`1` is the player plus up to two on-site hires and one support. Enforced only at hire time — `BarScreen` simply doesn't offer a posture once its slot is full — **not** at `crew_working`/tactical spawn time: an indefinite retainer hire still turns up in every job's fight regardless of that job's cap, same as before this existed. A different mechanism from `Scene.roles`/`Posture` above (which is descriptive, one row per beat, and unconnected to `CrewHire`) — this one is a hard cap on what you can hire, not a suggestion of who'd suit the beat.
...
**An info-broker fixer sidesteps the bar-luck meeting.** `fixer.RUNNER_BROKER_FIXER_IDS` (today just Neon Choir) adds a Contacts section to `FixerOffersScreen`: one row per active, unmet, available runner (`app.runners`, `runner_available`), priced by `runners.intro_cost` (`daily_cost * RUNNER_INTRO_COST_MULT`). Paying it calls `meet_runner` directly — no need to catch that runner `DRINKING` in person first. Recruiting terms themselves are unaffected; this only buys the introduction `BarScreen`'s `meet_<id>` step otherwise gates on luck.

**The run-time effect exists on the grid**: a hire fights beside you in a job's tactical stages (`crew_on_site`, `combat.crew_stats`, `_CREW_PROFILES` — see Tactical combat), where `rating` is finally read as their attack pool. **`crew_on_site`, not `crew_working`**: a hire taken on as remote support is on the job and takes a cut, but isn't at the location and gets no body on the map. `on_site` was write-only before that — set at hire time, capped by `max_on_site`/`max_support`, saved at v49, and then ignored when the fight opened, so a netrunner hired explicitly as support turned up at the building holding a pistol. What support *does* is Remote support, below. Abstract fights and non-combat stages still don't field them. Going down on a job has consequences that outlive the fight: `Character.dead_runners`/`arrested_runners` (save v44), written by `resolve_downed_crew` and read by `runner_available` — a killed runner is off the roster for the run, an arrested one for `ARREST_DAYS`. `runner_available` is a second gate beside `known_runners`: meeting someone is what makes them hireable, staying alive and out of a cell is what keeps them so. **Still to come**: assigning a hire to a `Role.filled_by`, and crew in `abstract_combat.py`'s fight.
...
**Recon is the Infiltrator specialist** — agility/perception/cool had none, unlike Netrunner (Intrusion, Data Heist). Every beat leads with a perception skill specifically (not just an Infiltrator-mapped one), which does double duty: it gives Infiltrator a contract of their own and it's the perception fix noted above. Otherwise built like any specialist archetype — the other two approaches per beat sit on different stats, so a non-Infiltrator can still take it and bleed through.

**Bodyguard is a different shape, not just a different theme.** It's the one archetype with no `COMPLICATION` row at all (always exactly `APPROACH -> OBJECTIVE -> EXFIL`, three beats, never four) and the one archetype where most beats have nothing to check at all: every stage carries `JobStage.vigilance`, and `generate_job` rolls `VIGILANCE_THREAT_CHANCE` (0.25) per stage — a miss becomes a `scene.Stage.narration` beat (see Runner-mode activity types) instead of the usual pool, so most legs of an escort just pass. `JobArchetype.hours_cost` (a per-tier override, `None` for every other archetype) also makes it a flat 4 hours regardless of tier — a genuinely low-time-commitment job, not just a short one this run. Leads with perception (a second Infiltrator specialist, alongside Recon) but strength/combat skills stay prominent as the way to actually deal with a spotted threat, not just spot it.

The load-bearing rule, enforced at import over the whole pool: **approaches in one stage must sit on different core stats** (a stage is a gate every build must pass; the same failure `corpmap_gen._filler_pool` guards against on legwork). Gigs are exempt (optional/self-selected). Simulated over the then-three presets × archetypes (before Gunslinger and Fixer, and before the skill table changed under all of them): **45–61% completion, under 2.2% deaths** (a failed stage still advances, costing health and the final reward, never the run). **These numbers predate Wetwork, Burglary, Data Heist, Bodyguard, and Recon** — re-run before relying on them with all nine archetypes.
...
Flow: pick a fixer (CorpMapScreen Fixers rows or `ContactsScreen`) → `FixerOffersScreen` → accept → moves from the fixer's board (freeing a slot for `refresh_offers`) into `Character.accepted_jobs`, appears in the activity list alongside its generated legwork. No `FixerListScreen`/`f` binding — fixers are list rows. `on_new_day()` drops expired accepted jobs; `expire_offers`+`refresh_offers` do the same for board offers. Completing a job (reaching a stage with no `next_stage`) removes it via `remove_job(scene.id)` — one-shot, unlike gigs/legwork.

## The Phone (`screens/info_screens.py` — `PhoneScreen` and its apps)

Everything the runner can reach *without being somewhere*, behind one hub. `PhoneScreen` replaced `ContactsScreen` as the top-level screen (`CorpMapScreen`'s `c` binding and its Phone category both push the Phone now; Contacts is one app on it). The home screen is a 3-column `layout: grid` of app tiles (`PhoneScreen.APPS`), each pushing **its own screen** rather than expanding inline — which is what keeps all four apps plain `BackScreen`s instead of one screen with four modes.

- **Contacts** — the pre-existing Fixers/Runners/Locals/Corps panel screen, unchanged apart from how it's reached.
- **Web** (`WebScreen`) — each corp's site as an app-shortcut row (→ `CorpWebsiteScreen`, see Corp websites), then a **Search** section: **a flattened cross-fixer job board.** Every `Fixer.open_offers` from every fixer the runner has established trust with (`trust_with(fixer.id) > 0`), in one list, accepted **inline** rather than by pushing `FixerOffersScreen`. Note what this is *not*: it isn't new off-site access — `ContactsScreen`'s Fixers panel already opens any trusted fixer's board from anywhere, on the same trust gate. What Search adds is not having to pick a fixer first, which matters once trust spans several boards. Reads `open_offers` not `offers` (a job an NPC runner took shouldn't pad the list — see Rival AI) and re-checks `taken_by` at accept time, since the day tick can mark one between renders.

  **`CorpMapScreen`'s Local-tab Fixers panel stays the location-gated route** (`fixer.location_id == character.location_id`) and is the only one that *discovers* anybody: `fixer.discover_fixers_here` is called from `CorpMapScreen` only, so standing in a fixer's district is still the sole way onto `Character.discovered_fixers`. Trust — hence both Phone routes — comes only after working with them. The Phone widens reach for contacts you've earned; it never replaces walking the map to find them.
- **Alarm Clock** (`AlarmClockScreen`) — sets `Character.alarm_hour`, the one thing on the Phone that touches the clock. Choices are `ALARM_HOUR_CHOICES` (every third hour) rather than a typed hour: every other screen in the game is `ListView`-driven, so a free-text field would be the single exception. Tapping the current alarm clears it. `app._rest_hours()` then shortens the next Rest to "until that hour" (see Time & the day clock), and `rest()` clears it afterward — an alarm is **one-shot**, not a standing wake time.
- **Messages** (`MessagesScreen`) — a read-only recap of what Contacts/Web already show (trusted fixers holding open job/security work, independent runners out `WORKING` and on what), derived at display time. No state of its own, so there's nothing here to persist or expire — same shape as `CorpWebsiteScreen`.

`SAVE_VERSION` 39 added `Character.alarm_hour`, the Phone's only persistent state; every other app renders from what the save bundle already holds.
...
- **critical failure** — costs the *same* health as a plain failure, but ends the contract immediately: standing/trust/rep move against the runner, `CorpMapScreen` drops it. No fight, no combat routing.

`security.resolve_security_night` returns a `NightResult` (roll, pay, bonus, blown, completed) so `CorpMapScreen` builds `notify()` text from data. **Not yet balance-simulated** — `NIGHTLY_PAY_BASE` (35/50/70) sized against `RivalRunner.daily_cost` (45/55/60); `NIGHTS_RANGE` (3–5) × pay + a 50% completion bonus lands near a job's `REWARD_BASE`, spread slower/safer but immobilizing (can't work elsewhere without abandoning progress) — a judgment call, not a simulated one.

## Combat (`shadowguy/combat.py`, `shadowguy/abstract_combat.py`, `screens/combat_screen.py`)
...
**Two modules, split by who needs which half.** `combat.py` is what a fight is *made of* — the enemy roster (`Enemy`/`ENEMY_TIERS`/`roll_enemies`/`NPC_WEAPONS`), `crew_stats`, the hit formula (`resolve_hit`/`soak_damage`), `player_defense`/`player_soak`, the reach bands (`MELEE_RANGE`/`THROWN_RANGE`/`FIREARM_RANGE`/`weapon_range`, moved down from `tactical.py` once `Enemy.reach` needed them), `equipped_weapons`/`consumables_with`/`combat_consumables`, `attack_verbs`, `smartlink_bonus`, `Drop`/`drop_for_result`/`FREE_ROUND` (the drop's payout, beside the thing it pays out for), and the shared `roll_tier_pool` that both `roll_enemies` and `matrix.roll_ice` draw a fight's opposition through. It runs no rounds. **All three fight surfaces import it** — `abstract_combat.py`, `tactical.py`, `matrix.py` — and none imports another, which is the layering that stops a grid attack and an abstract one from drifting into different formulas. `abstract_combat.py` is fight surface 1 on top of it: `CombatState`/`Fighter`, `Action`/`ActionKind`/`available_actions`, `start_combat`/`take_turn`, the six action handlers, `CombatOutcome`, and the round's tuning constants (`BRACE_*`, `READ_*`, `INTIMIDATE_DIFFICULTY_BONUS`, `FLEE_*`). Only three screens need that half — `combat_screen.py` for the whole round loop, `scene_screen.py` and `corp_map_screen.py` for `CombatOutcome` alone. `Enemy` deliberately stays in `combat.py` rather than moving with the round loop — it's reachable from a pickled `scene.Encounter`, so moving it would invalidate every save for no gain.

**A fight is a `Stage`.** `Stage.combat` holds an `Encounter` (prompt, enemies, victory/escape `Outcome`), routed to by an ordinary `next_stage`. A stage is *exactly one* of choices, a fight, or a tactical map, never mixed (guarded in `Scene.__post_init__`) — a combat stage's "choices" are `abstract_combat.available_actions`, from the runner's gear/skills, not the scene.
...
**Actions deliberately span five of the six stats**: attack (weapon-dependent skill — Agility armed, Strength's `Grapple` bare-handed), **brace** (Toughness/Body), **read the fight** (Tactics/Logic, banks a next-attack bonus), **face them down** (Intimidation/Cool, breaks the weakest enemy's nerve), **break and run** (Dodge/Agility), plus a row per grenade. Bare hands, bracing and running are always available. **Perception is the stat a fight no longer touches** — it used to enter through `Firearms`, which is now agility's `Pistols`/`Automatics`/`Longarms`; nothing in `available_actions` rolls it.
...
**Weapons are the damage, skills are the hit** — with one deliberate exception: **Strength is added to the damage of a melee or thrown hit**, full stat, 1:1, uncapped (`combat.melee_damage_bonus`). `skill_value` still decides connection and `shops.Item.damage` is still the only place a weapon's profile is written; Strength rides on top of the base damage. Qualifying is derived, not tabulated — anything that isn't a `shops.RANGED_SKILLS` weapon, so blades, clubs, bare hands and thrown knives get it while guns and bows do not.

Threaded like `smartlink_bonus`: the two **player** attack sites (`abstract_combat._attack`, `tactical.player_attack`) pass it into `resolve_hit`'s `base_damage` rather than it living inside `resolve_hit`, because that function is the shared formula for both directions of a fight and takes final numbers, knowing nothing about who is swinging. **It is no longer player-only** — `melee_damage_bonus` takes anything with a `.stat()`, and since an `Enemy` now carries the same six `CORE_STATS`, `Enemy.damage` folds it in the same way. A thug's pipe gets its owner's Strength, and so does a hired runner's blade.

**This is a large, deliberate buff and is not balance-simulated.** Enemy health tops out at 10 (`ENEMY_TIERS`' Chromed Enforcer and Sec Heavy) against a 4–7 base damage melee weapon, so a Strength 6 runner one-shots most of the roster with a katana, and at the top end Strength alone out-damages every weapon in the catalog. It also makes `UNARMED` (base damage 0) a real attack rather than a fallback: bare hands at Strength 6 out-damage a pipe pistol. That's what Strength buys — it carries only two skills (see Stats and skills), and this is the compensation.

### The enemy roster

**An `Enemy` is written on the player's own sheet.** Six `CORE_STATS`, invested `ranks`, a `weapon` and an `armor` rating — and every number a fight consumes is a *derived property* over those, through the same functions the player goes through:

| derived | formula | player counterpart |
|---|---|---|
| `attack` | `skill_value(self, weapon.skill)` | the same call |
| `defense` | `DEFENSE_BASE + skill_value(self, "dodge")` | `player_defense`, same base |
| `damage` | `weapon.damage + melee_damage_bonus(self, weapon)` | the same two terms |
| `toughness` | `body + armor` | `player_soak`, minus cyberware |
| `health` | `body * HEALTH_PER_BODY` | `BASE_HEALTH + body * HEALTH_PER_BODY` |
| `reach` | `weapon_range(weapon)` | the same call |
| `stun_damage` | `weapon.stun_damage` | the same field |

The derivation is *literal, not parallel*: `skills.skill_value` imports `Character` only under `TYPE_CHECKING`, so it duck-types onto anything exposing `.stat()`/`.skill_rank()`/`.skill_gear_bonus()` — three methods on `Enemy` is the whole bridge. Add a stat, a skill or a weapon effect to the player and an enemy holding the relevant thing gets it too. This replaced seven hand-set scalars that meant a Corp Sec's accuracy and a runner's accuracy were unrelated numbers being compared against each other (save v54).

**Two places the symmetry deliberately stops.** `health` omits `BASE_HEALTH` — that flat +10 is the protagonist's alone, and handing it to every mook would triple the roster. And an enemy has no cyberware and no `skill_gear_bonus`: its gear is a weapon and an armor number, nothing finer. `armor` is a plain int rather than worn `Item`s because nothing loots it yet.

**NPCs are armed from `combat.NPC_WEAPONS`, not `shops.CATALOG`** — hand-built like `UNARMED`, and necessarily so: every one sits *under* `shops.MIN_WEAPON_DAMAGE` (4, the floor for anything purchasable). That gap is what keeps the roster's damage where it was tuned, since enemy damage now picks up Strength like everyone else's. No NPC weapon declares `recharge_rounds`: cooldowns are tracked on the player's state only, so an NPC one would be silently ignored by both fight surfaces.

**The roster is 11 entries across 3 tiers, and the spread is *shape*, not power.** Within a tier they differ by what they punish — something to kite (Street Lookout, Marksman), something that punishes standing still (Juicer, Chromed Enforcer), something that can barely be hit (Razorgirl), something that soaks (Bulwark), and the Shock Trooper, the only thing in the game that threatens a **knockout** rather than a death by driving `Character.stun` (which carries between fights).

**Note nearly every weapon skill sits on `agility`, the same stat as Dodge** — so an enemy's accuracy and its evasiveness move together. That coupling is the main constraint on any new row, and it's why the two entries that break out of it sit at opposite ends: the Bulwark buys armor instead of agility, the Razorgirl agility instead of armor.

**Calibration, and the one thing that could not be held.** The five originals keep their previously tuned `attack`, `damage` and `toughness` *exactly*, and their `health` within 2, because those were fit against a balance sim that **is not in this repo**. `defense` could not be held: it was 9–13 hand-set, and `DEFENSE_BASE` alone is 12, so deriving it floors every enemy at 13. The roster is correspondingly ~1.4 dodge dice harder to hit than before — which falls hardest on builds with no weapon skill, since a fresh `Character` rolls 2 attack dice against what is now a 2-dice dodge pool. Measured against **`tools/combat_sim.py`** (abstract fights, flee at 35% health, 4000 trials per cell), deaths held: Enforcer 0% at every tier, Gunslinger 0%, Hacker 0.1% / 4.1% / 34.3% by tier against a 0% / 1.9% / 31.5% baseline. Fights run ~1 round longer, and tier 1 gained a **5.5% knockout rate** for the Hacker that did not exist before — the Shock Trooper's stun is a new failure mode, not a re-tuning of an old one. **Re-measure anything you change here.**

**Stun is the harshest number in the roster for its size, and is tuned down accordingly.** It bypasses the soak roll, it persists between fights (`Character.stun`), and the KO threshold is `stun >= current health` — so the bar *falls to meet* the accumulating total as a runner is hurt, rather than the total having to climb to a fixed bar. At `stun_damage` 2 the Shock Trooper put a Hacker down 72% of the time it turned up in a pair, at half health, **with the flee threshold never tripping**: a loss condition the escape valve doesn't watch for is close to the cage `abstract_combat`'s flee rules exist to prevent. At 1 that is 51% in the same isolated worst case, the runner does now sometimes get out, and the tier-1 figure is the 5.5% above. Treat any future stun weapon as a flee-rules change, not a damage-table one.
...
**Balance, simulated over the then-three presets × job archetypes (best-odds approach, running below 25% health):**
...
## Tactical combat (`shadowguy/grid.py`, `shadowguy/tactical.py`, `shadowguy/tactical_gen.py`, `scene.TacticalStage`, `screens/tactical_screen.py`)
...
**The module split mirrors combat exactly.** `scene.TacticalStage` is the grid analogue of `Encounter` (holds `Outcome`s, routed by `next_stage`); `TacticalScreen` is the `CombatScreen` counterpart.

**Space itself is one layer further down, in `grid.py`**: `Grid`/`Tile`, the tcod FOV (`_fov`/`has_line_of_sight`/`visible_tiles`), A* (`path_between`), move legality (`step_neighbors`) and the range metric (`chebyshev`) — no units, turns, weapons or game state of any kind, and it imports nothing from the package. `tactical.py` is the fight over it: units, turn order, cover, aiming and the AI phases. **That's what lets the burglary arrow point one way**: `buildings.py` needs only the geometry, `tactical.py` needs a whole `Building` to walk one, so the chain is `grid → buildings → tactical` with nothing reaching backwards. (It used to be `buildings → tactical` with `tactical` reaching back for `Building`/`Lock` under a `TYPE_CHECKING` guard; extracting `grid.py` removed the cycle and the guard with it.) Moving `Grid` cost save v50 — pickle resolves classes by module path, and a `Grid` hangs off both `scene.TacticalStage` and every `buildings.Level`.

**Two more modules split off the fight, both one-way and neither costing a save version** (nothing either owns is reachable from a pickled bundle — only `combat.Enemy`, `matrix.Ice`/`MatrixNode`/`MatrixNetwork`, `grid.Grid` and `buildings.*` are). `tactical_gen.py` holds the BSP generator (`TacticalMap`/`generate_map` and the carving helpers) and is a **leaf** — grid.py plus tcod, nothing else, and emphatically not `tactical.py`: a map knows nothing about units or turns, which is what makes it generatable ahead of the fight and picklable onto a `TacticalStage`. That's the same `corpmap.py`/`corpmap_gen.py` split, for the same reason. `support.py` holds the remote hacker and points the *other* way: it imports `tactical.py` (for `TacticalState` and `raise_alarm`) and `tactical.py` imports nothing back at runtime, because the fight only ever *carries* a `Support` and touches two of its plain attributes (`acted` in `_begin_player_turn`, `blinded_cameras` in `check_detection`). Every support entry point is called by a screen, never by the engine mid-turn — that's the property that keeps the arrow clean, and the one to preserve if support ever grows.

**LOS and range are separate gates, deliberately.** `has_line_of_sight` is unlimited-radius FOV obstruction; weapon reach is an explicit distance check (`weapon_range`) — tcod's FOV radius is Euclidean and excludes the exact-radius cell, so reading range off it would be off by one. Guns kite; melee has to close. **Reach is three bands, derived from the weapon's skill and no new `Item` field**: `shops.RANGED_SKILLS` (the gun categories plus `archery`) gets `FIREARM_RANGE` 8, `throwing` gets `THROWN_RANGE` 4, everything else `MELEE_RANGE` 1. The bands and `weapon_range` itself live in **`combat.py`**, not here — `Enemy.reach` reads them too, and that side of a fight has no `Character` behind it. The middle band exists because categorizing weapons made "ranged or not" too coarse — a thrown knife shouldn't cover a rifle's ground. Consequence of reach following the *skill*: the tasers, now `Pistols`, shoot 8 tiles where as `Misc Weapons` they reached one. That made `Item.recharge_rounds` load-bearing here, and it had been **inert on this surface** — set and read only by `abstract_combat`, so a taser paced itself in an abstract fight and fired every turn on the grid. `TacticalState.weapon_cooldowns` now mirrors `CombatState`'s exactly (set in `player_attack`, ticked in `_begin_player_turn`, gated in `can_hit` — the one place weapon reach is answered, so a cooling weapon drops out of `targets_for`/`weapon_for_target`/`attack_targets` and the Attack tile together). **`stun_damage` is still inert on the grid**: `tactical.py` has no stun meter, so a taser there does only its `resolve_hit` margin as ordinary damage.

**Movement is eight-way** (`grid._STEPS`, and `path_between`'s A* at `diagonal=1.0`), a diagonal costing the same single move as a cardinal one — which is what `chebyshev` has always said a diagonal is worth, so movement range and the metric behind melee reach, grenade blasts and cover adjacency finally agree. **Both sides step through the same table**: the AI closes diagonally too, so nobody outruns anybody by walking corner-wise. The player drives it from the arrow keys or the numpad — `TacticalScreen` binds `1`-`9` (a terminal delivers the keypad as plain digits with NumLock on) and `home`/`pageup`/`end`/`pagedown` (the four corners, with it off), all through the same `action_move`, so the diagonals steer the aim cursor exactly as they steer the runner. A one-cell-thick wall is still a wall — a diagonal step *into* it lands on the wall cell — so doors (and the locks on them) remain the only way between two rooms.

**The turn model**: move up to `speed` tiles, then one action; `end_turn` then charges the scheduler for what that turn cost and lets allies/enemies take their own scheduled turns before the next player turn opens (see Turn-order scheduler below — this replaced a fixed "ally phase, then enemy phase" pair that ran unconditionally every End Turn). **`moves_left` only rations anything while `TacticalState.in_combat`** (`state.alarm` or any live enemy `alerted`) — an ordinary tactical fight is this from the first turn (`Unit.alerted` defaults to `True`), so nothing changes there. A burglary's guards start unalerted, and walking a quiet house before `check_detection`/`raise_alarm` catches you spends no `moves_left` at all (`legal_moves`/`move_player`/`take_stairs`/`attempt_lock` all gate the decrement on it) — only the fight that follows rations movement, matching the guard's own turn (an unalerted unit is filtered out of the scheduler's candidates outright, so there was never anything to react to). The step that gets you spotted is itself free, since nothing was hunting you when you took it; `in_combat` flips (from `check_detection` inside the same call) before the *next* step, not the one just taken. One-way, same as `alarm` — nobody un-notices you. **Enemy reach is per-enemy** (`combat.Enemy.reach`, tactical-only) and now *follows the weapon in its hands* — `weapon_range` of whatever it carries, the same call the player's reach goes through: armed guards shoot from `FIREARM_RANGE` 8, street muscle and the bruiser close to `MELEE_RANGE` 1. Guards previously shot from a hand-set 6; a gun that reached less far in NPC hands than in the player's was exactly the second source of truth the derived stat block removes. **Every unit runs the same body** (`_take_unit_turn` over `_advance_and_attack`): pick a target, close via A* until it's in reach, then hit it — a ranged unit holds distance because it stops the moment it has a shot, melee has to walk all the way in. **Fleeing is positional and always works** — reach an exit tile, no roll, no parting shot — the same law as `abstract_combat.py`'s flee, enforced spatially.

**Turn-order scheduler** (`Unit.quickness`/`Unit.next_turn`, `combat.QUICKNESS_BASE`/`player_quickness`/`Enemy.quickness`, `tactical._time_cost`) — a classic-roguelike replacement for the old fixed-phase model: how often a unit's turn comes up is now driven by quickness, not a guaranteed once-per-End-Turn slot. `quickness` is `QUICKNESS_BASE` (10) + Agility — `player_quickness(character)` through `Character.stat()` (gear/cyberware/fatigue/Humanity all feed it, like every other stat), `Enemy.quickness` off the raw field (no bonuses, same asymmetry as `toughness`/`defense`). Deliberately a *separate* concept from `speed` (tiles moved per turn stays flat, unchanged) — quickness is turn *frequency*, not stride length. Realistic range: player 11 (base) to ~18 (heavy Agility build + reflex_coprocessor); roster 11-13 (enemy agility 1-3). First-slice, not balance-simulated, like the rest of this surface's numbers.

`end_turn` charges the player's `next_turn` by `_time_cost(quickness, work)` — `work` is tiles moved plus one more if the turn's action was spent too, floored at 1 (an empty End Turn still passes real time, or spamming it would be a free way to skip everyone else's turn forever) — then loops: any alerted ally/enemy whose `next_turn` is **strictly less than** the player's new `next_turn` takes its own turn (`_take_unit_turn`, always exactly 1 work unit — one combined close-and-attack), advancing its own `next_turn` the same way, until nobody's left eligible. Strict `<`, not `<=`, matters: with `<=` a unit whose per-action cost divides evenly into the player's committed time re-qualifies the instant it lands on that exact tick, silently doubling its actions in the *equal-quickness* case — the common one, since most of the roster sits at `QUICKNESS_BASE + 1`. A genuinely quicker unit (higher Agility) can still legitimately earn a second action before the player's next turn; a slower one can sit a turn out entirely. `min(candidates, key=...)`'s stable sort means an exact tie still favors allies (list order), matching the old model's "allies go first" bias in the one case it still applies.

Every branch of `_take_unit_turn` — stunned, no target, or a resolved attack — falls through to the same trailing `next_turn` advance; none of them return early. A unit whose turn doesn't cost it scheduler time would stay permanently eligible and spin the `while` loop forever the next pass — concretely, a stunned unit re-selected within the same `end_turn()` call before its `stunned_rounds` had really elapsed, or (worse) an ally that's cleared every guard on a burglary's *current* level while other floors still hold guards, whose `pick_target` returns `None` forever with nothing to stop the loop revisiting it. `stunned_rounds` keeps its old meaning (skip N of *your own* upcoming turns) — now naturally paced by however often the scheduler grants that unit a turn, rather than uniformly every End Turn.

**A unit joining the schedule after combat has already been running needs catching up to the current time, not left at its construction-time `next_turn` of 0** — two places do this, both critical: `raise_alarm` sets every newly-alerted unit's `next_turn` to the player's current one (on-board and everyone still waiting in `off_level_units`), and `enter_level` does the same (via `max`, never backward) for anyone `alerted` among the units swapping onto the board. Skip either and a guard who's sat frozen at time-zero through a long sneak floods the scheduler with a burst of catch-up turns the instant it's finally live — an unfair pile-on, not a fast enemy legitimately earning extra actions.

**A hired runner fights on the grid** (`Character.crew_working(job_id)` → `combat.crew_stats` → `start_tactical(allies=...)`, wired at `SceneScreen._show_tactical`). Read off the `Character` when the fight opens, not baked into the stage at generation, so a hire made after accepting the job still turns up; for-job hires plus everyone on retainer (wider than `crew_for_job`, which is who takes a *cut*). They spawn ringing the player (`ally_spawns`; any that don't fit the entry room sit it out), are **AI-driven** on their own phase — a hire you have to micromanage is a second character, not backup — and are **never targetable by the player's cursor** (`enemy_at` only reads `Side.ENEMY`), so friendly fire is impossible by construction rather than by a rule. A downed hire is a body you walk over, and **does not end the fight**; only the player's death does.

**A hire at zero health is down, not dead — what kills them is how the fight ends.** `stabilize_ally` costs the turn's action and one `EffectKind.HEAL` consumable to stop their bleeding from an adjacent tile (`s`; `stabilize_targets` gates on kit + adjacency). It is **first aid, not a revival** — they stay out of the fight, so there's no knock-down/pick-up loop, and spending a whole turn standing over a body under fire is the decision. Deliberately *not* gated on `health_kit_used_today`: that cap is about a runner topping themselves up, not who they can patch. `resolve_downed_crew` then settles each one from `_end_fight` — the single place a fight ends, so a hire's fate follows from the fight ending rather than from whoever renders or dismisses the screen — off two facts — stabilized, and whether you **held the field** (`VICTORY`, so you can carry them out) or walked out an exit (leaving them for whoever arrives next). Odds live in `_CREW_FATES`: stable + won is always `RECOVERED`; unstabilized + won risks bleeding out (25% `KILLED`); stable + escaped is mostly `ARRESTED` (60%); left bleeding on someone else's floor is 40/40 killed/arrested. **`KILLED` is permanent for the run** and `ARRESTED` holds them `ARREST_DAYS` (7); both discharge the hire and come off the roster via `Character.runner_available`, which `BarScreen` and `rivals`' daily runner turn both now consult. A player death skips the whole thing — the run's over. First-slice odds, not simulated. Grenades still hit `state.enemies` only. Crew stat blocks are `combat.Enemy` instances (`Enemy` is really "a combatant who isn't the player" — one dataclass, one `resolve_hit`; `Side.ALLY` is what says which way they point), built from `RivalRunner.rating` + a per-archetype stat sheet (`_CREW_PROFILES`) on the same six stats every other `Enemy` uses, by *buying rank* in their weapon's skill rather than being assigned, so a better-rated hire is a better-trained one. **`rating` is a runner's standing on the street, not their marksmanship**: it reaches the attack pool as `rating - combat_gap`, which is 0 only for the Solo — the one archetype you hire to shoot people. Without it a Netrunner shot exactly as well as a Solo of equal rating, which is not what you hire a netrunner for. Their defense rose with everyone else's when it became `DEFENSE_BASE + Dodge` (the old `CREW_DEFENSE_BASE` of 8 is gone), which is a buff to hires. Tactical only — `abstract_combat.py`'s fight has no positions to put a second body in. **Not balance-simulated**: the sim below assumes a lone runner.

**Everyone chooses targets, and cover is how you steer them** (`pick_target`, re-picked per unit per round). Ranked by `cover_bonus` between them and that enemy, then distance, with no-line-of-sight sorting last; the player breaks exact ties, so a hire is never a quiet meat shield. Because the pick happens *before* movement, an enemy crosses the room toward the target it wants rather than the nearest body — and a runner who steps out of cover draws the next one's fire. One ranking, one function (`pick_target`) — an ally aims by it too, pointed the other way, so the mechanic can't drift between the sides. It costs one FOV pass per unit, not one per candidate.

### Remote support (`shadowguy/support.py`)

**The other half of a crew.** An on-site hire is a body on the board with a `combat.Enemy` stat block; a support hire never appears on the map at all and instead does things to the *building* on the player's order (`support.Support`, `support_for`, `support_tasks`, `run_support_task`; the `h` key and `HackerPickScreen`).

**Support is a role, and the gate is gear rather than a label.** `runners.can_work_support` asks one thing — does this runner own a cyberdeck (`RivalRunner.deck_id`) — and `Character.hire_for_job` refuses the posture to anyone who doesn't, rather than selling a cut for an empty comm link. Reading `archetype` instead would mean a netrunner who owns nothing still counts and a solo holding a deck still doesn't, and neither is true. **The deck gates twice**: it decides whether they can work support at all, and its `program_slots` caps how many support programs they carry, so a Burner Deck runs one thing however good its owner is. **Riggers are the other half and don't exist yet**: when they land they gate the same way, on owning drones rather than on being called a rigger. A `deck_id` is validated against `shops.ITEMS_BY_ID` at import — a typo would otherwise fail silently by making a roster hacker quietly unhireable.

**They act on their own turn, not yours.** Directing them costs no move and no action — that's the entire reason to pay for a second person rather than a better gun, and it's what makes the tile worth a key of its own. One task per player turn (`Support.acted`, cleared in `_begin_player_turn`).

**What stops that being free is the trace.** Every task rolls the hire's `rating` (which is what `RivalRunner.rating` was always for — "an effective skill_value the run-time crew effect will roll") against the program's difficulty. A success costs nothing: getting in and out clean is the whole skill. A miss climbs the meter by that program's `trace_on_failure`, and at `TRACE_CAP` (10) the building's ICE has them — they drop offline for good *and* the noise raises the alarm. So the pressure is "how hard do you push them", the same shape as `matrix.MatrixState.security`, rather than a resource counted down.

**Tasks come from `runners.SUPPORT_PROGRAMS`**, hand-built and not for sale for the same reason `combat.NPC_WEAPONS` is: `shops.Program` is written entirely in matrix-fight terms (ICE damage, sleaze, extract) and has no field that could mean "scout that room". `min_rating` gates each one, which is what makes a better netrunner worth paying for beyond the roll itself.

| program | rating | difficulty | trace | what it does |
|---|---|---|---|---|
| Probe | 0 | 11 | +1 | Reveal the nearest room with anything unseen in it, and say how many cameras cover it |
| Wrench | 5 | 14 | +2 | Open the nearest locked door, or blind the nearest live camera — one task, because from the far end of a link they're the same job |
| Ripper | 7 | 17 | +3 | Reach through the nearest guard's own chrome: `CYBERWARE_STUN_ROUNDS` of `Unit.stunned_rounds`, the same field a Flash grenade sets |

**A burglary is where support earns its cut** — locks, cameras and unalerted guards are exactly what a hacker in the system reaches, and `max_support` is 1 for Burglary and Wetwork and **0 for Data Heist** (that job is the netrunner's own; you're the hacker on it). `support_tasks` lists only what has something to point at right now, so the menu greys out rather than lying. Blinded cameras live on the `Support` rather than mutating `Building.cameras`, because the building is pickled inside the accepted job and a camera should come back when the job is re-walked; an opened lock does mutate, exactly as `attempt_lock` does. **First-slice numbers, not balance-simulated** — `tools/combat_sim.py` drives the abstract surface and sees none of this.

**Maps are generated** (`tactical_gen.generate_map`): a BSP partition (tcod, seeded off the caller's rng via a derived int for reproducibility), rooms + corridors with scattered low cover, player entering by `exits` at one end, squad at the other. **Retries until every enemy spawn and exit is reachable** (raises rather than hand back an unplayable fight). Maps are `TAC_MAP_WIDTH`×`TAC_MAP_HEIGHT` (30×10), sized to fit `TacticalScreen` at 80×24 without scrolling. Cover density is softly themed by site `LocationKind` (`jobs._cover_density`, `.get`-with-default, no import guard).

**The grid renders as connected line-art, not a field of `#`.** `screens/__init__._terrain_glyph` maps a `Tile` to (glyph, colour) — floor `·`, low cover a shaded `▒` in its own colour ("hide here" needs to read at a glance), and a wall through `_wall_glyph`, which builds a 4-bit N/S/E/W neighbour bitmask and indexes `_WALL_CONNECTORS` for the box-drawing character that connects in exactly those directions. So a room reads as an outlined floor plan. Two deliberate non-junctions: mask 0 (an isolated wall) is `■` and mask 15 (wall on all four sides — solid rock inside a wall band, not a crossing) is a plain `█`. Out-of-bounds counts as wall (`_is_wall`), which is what closes the map's own border instead of fraying it. **None of this touches `tactical.py`/`grid.py`** — it's presentation over the same `Grid`, which is why a burglary interior shares the helpers verbatim and the two surfaces look like one game (they are literally the same screen — see Burglary jobs).

**Fog of war: a tile you've never had FOV on is blank; one you've seen before but can't presently see renders dimmed, not hidden.** `TacticalState.explored` (a `set[Coord]` per level index — a burglary's levels are separate grids) accumulates every tile that's ever been in the player's FOV, folded in by `_reveal` at the start of each player turn (`_begin_player_turn`) and at each of the three places `state.player.coord` actually changes (`move_player`, `attempt_lock`'s success branch, `enter_level`) — not from `_settle`, which runs after plenty of turns where the player didn't move. Dimmed remembered tiles still show terrain and static features (exits, stairs, the objective, locks, cameras) — those don't move. Units do: an enemy, ally, or corpse only renders while currently in FOV, since memory of a tile says nothing about who's standing on it now. `grid.visible_tiles` hands back the whole `[y, x]` FOV bool array in one call for the current-sight half of this; a renderer must **not** call `has_line_of_sight` per tile — that recomputes the FOV from scratch for every cell on the map. No exceptions carve into this yet — a legwork mechanic to know a building's layout or guard positions ahead of time is a real future direction, not built.
...
**Every action is aimed, through one cursor.** `TacticalState.aim_cursor` plus `aim_kind` (`AimKind.ATTACK`/`GRENADE`) is a small modal turn state: while it's set, arrow keys drive the cursor instead of the player (`move_aim_cursor`), Tab snaps it to the next enemy worth aiming at (`snap_aim_to_next_target`, nearest first, wrapping), Enter resolves it (`confirm_aim`, the one place the attack/throw split is decided — not the UI), Esc backs out (`cancel_aim`). **Backing out is always free** — nothing but a resolved attack or throw sets `acted` or pops a consumable — and confirming on an illegal cell returns False and *keeps aiming* rather than eating the action. `aim_is_legal` is the single legality call a renderer needs: green cursor means confirming resolves, red means it won't. `f` opens the cursor on `best_shot`'s target, so the old one-keypress attack is still `f`+Enter.

**Which weapon fires follows from the target, not a second menu** (`weapon_for_target`: the hardest-hitting equipped weapon whose `weapon_range` and LOS reach that enemy, via the shared `can_hit` gate). The player picks *who*; range picks *what* — a knife only at arm's length, the gun for anyone further out. `best_shot` answers only *who* — the nearest of `attack_targets` — because the weapon follows from that answer; the default and an explicit aim therefore can't drift into two policies. `TacticalScreen`'s Attack tile names both while aiming (`Pipe Pistol → Enforcer`), which is where the player sees a step backward swap their blade for the gun. The log reads off the weapon too — `combat.attack_verbs` maps a weapon's *skill* to its (miss, hit) verbs (`fire on`/`shoot`, `stab at`/`stab`, `swing at`/`slash`, …), shared by both fight surfaces so a katana never "fires" and a pistol never "swings". Flavor only, keyed on skill rather than item id so a new catalog weapon reads right immediately, with a generic fallback pair rather than a raise for an unknown skill.

**Grenades are the one action aimed at a tile, not a unit** — the same cursor, a different legality rule (`legal_grenade_target`: `GRENADE_RANGE` + LOS against any cell, vs. an attack needing a standing enemy some weapon reaches). `throw_grenade` reuses `abstract_combat._throw`'s three `shops.EffectKind` branches (`combat_consumables`, promoted public for exactly this reuse), but `COMBAT_STUN`/`COMBAT_DAMAGE_ALL` land as a blast — every enemy within `GRENADE_RADIUS` (chebyshev distance 1, a literal 3x3 square) of a target tile, not everyone standing, since the grid gives a grenade something abstract combat doesn't have: a place to actually miss. `COMBAT_ESCAPE` (a smoke grenade) stays positionless, same as the abstract fight — walking out isn't aimed at anyone, so it's the one throw that resolves with no tile at all. `begin_grenade_aim` starts the cursor on the player's own tile (always legal at range 0) and stashes the `pending_grenade_index`; the throw's 3x3 blast is shaded on the map while aiming, which an attack has no equivalent of. First-slice tuning, like the grenade catalog itself (`shops.py`'s `_CONSUMABLE_ROWS` comment) — `GRENADE_RANGE`/`GRENADE_RADIUS` aren't yet balance-simulated.

**Balance — far swingier than abstract combat, by design.** A tier-2 Hacker dies **~68% rushing with a monoblade** but **~10% kiting with a pistol** (abstract baseline 25%) — ranged is *safer* on the grid, melee-only crit-failing into a high-tier fight is real danger. **Left as-is on purpose** — fleeing is always available, and ~68% is a deliberately naive rush. Ranged guards pressure the rusher more than the kiter — though the reach gap that used to describe (firearm 8 vs guard 6) is **gone**: guards now shoot 8 as well, since reach derives from the weapon. The note already said the kiter's safety is a damage-economy fact rather than a positioning one and that bumping guard reach barely moves it, which is the reason this was allowed to change — but it is now unverified on this surface. Re-run the sim before touching flee/exit rules, `TACTICAL_FIGHT_CHANCE`, enemy counts, or reach. **These numbers predate eight-way movement** and haven't been re-simulated against it: both sides gained the same diagonals, so the kiter/rusher gap should hold in shape, but the absolute figures are now unverified.

## Burglary jobs (`shadowguy/jobs.py`, `scene.BurglaryStage`, `screens/burglary_screens.py`)

A **fourth kind of stage**, the second to break the "text list of Choices" mold: replaces a job archetype's whole APPROACH stage with a two-phase UI — pick a labeled entrance on a diagram (`EntrancePickScreen`), then walk a generated interior (on `TacticalScreen` itself — there is no separate walk screen) from that entrance's spawn to an objective tile, avoiding static guards. Burglary is the 6th `jobs.ARCHETYPES` entry (generic, mixed leads, no specialist) and the one this section is named for; every other stage of Burglary itself is an ordinary `Choice` list. Wetwork's APPROACH (the Solo specialist job, `jobs.WETWORK_STRUCTURE`) gets the identical treatment — `JobStage.burglary` is set for both — except its structure is always `COMPOUND` regardless of the job site's own kind, since the target is holed up in their own place, not the site's: see Wetwork's own comment in `jobs.py` for why its APPROACH pool reads as short entrance captions ("Front Gate") rather than sentences, the one place it departs from every other Wetwork stage's voice.

**The walk itself costs no `moves_left` at all until a guard, camera, or a botched lock makes it a fight** (`TacticalState.in_combat`, see Tactical combat above) — the same `speed`-gated movement every other tactical fight uses, just dormant while every guard is still standing their post. Rationed movement only starts the instant `check_detection`/`raise_alarm` catches you, which is also the instant sneaking stops being an option.
...
**The interior is a building, not a room** (`buildings.py`). A burglary target is a
`Building`: several `Level`s, each its own `Grid`, joined by `Link`s (a stair, a lift, a
door between two warehouses). Deliberately a **graph, not a stack** — an office tower is a
line of links and a compound is several ground-level grids side by side with a couple
...
chair in the wrong cell seals a room as effectively as a wall.
...
expires — it is **never a `corpmap.Location`**, so a target adds nothing to the map the
player walks around and no UI noise unless they hold the job. That's why `BuildingKind`
lives in `buildings.py` rather than beside `corpmap.LocationKind`: that enum answers "what
does this place sell" for a permanent fixture, this one "what is this building shaped
like" for something temporary.

**The interior is a tactical fight you're trying not to start** (`tactical.start_burglary`,
played on `TacticalScreen` — there is no separate walk screen any more). Three things make
it a burglary rather than a shootout:

- **Levels.** `TacticalState.grid` is whichever level is on the board; everyone else waits
  in `off_level_units` until you walk to them, so a guard left behind is exactly as you
  left them — hurt, alerted, or none the wiser. `take_stairs` (`>`) costs a move, like any
  other step.
- **Guards stand their post.** `Unit.alerted` starts False for them, and an unalerted unit
  sits out the AI phase entirely — that is what makes sneaking past one possible.
  `check_detection` runs after every move and every stair, off `GUARD_SIGHT_RANGE` + line
  of sight; `raise_alarm` then wakes *the whole building*, because a shout carries. Taking
  a shot raises it too, seen or not.
- **The score is the win.** `TacticalOutcome.SECURED` (reaching `objective`) is the only
  success. Clearing every guard does **not** end a burglary — `_settle` skips its VICTORY
  branch whenever there's an objective — so a cleared house is a quiet house, never a
  finished job. Every entrance is an exit; leaving by one is `ESCAPED` and applies
  `BurglaryStage.bailed`.
...
- **Integrity, not health.** `player_integrity` = `BASE_INTEGRITY + INTEGRITY_PER_LOGIC * stat("logic")` (gear-included) is a per-run pool, never touching `Character.health`. Draining it **ejects** (`MatrixOutcome.EJECTED`), doesn't kill — **there is no death in the matrix** (losing blows the contract via the same blown-job escape Outcome, never the run).
- **Logic's actions, not the six-stat spread** (matrix is the Hacker's arena on purpose): **Breach** rolls `cybercombat` (deck damage), **Harden** rolls `tinkering` (brace), **Analyze** rolls `infer` (read), **Jack out** always works (the escape valve). A non-hacker can fight here but bleeds.

**Three logic skills split the wire by what you're doing on it**, each owning specific cyberdeck programs and actions rather than one skill covering everything — named as constants at the top of `matrix.py` so a program's action is what picks the skill:

| roll | skill | why |
|---|---|---|
| Breach / ATTACK (`ATTACK_SKILL`) | `cybercombat` | fighting the ICE |
| Sleaze program (`SLEAZE_SKILL`) | `hack` | offensive web work — talking your way past a node |
| Extract program (`EXTRACT_SKILL`) | `computer` | pulling the file: data, not a fight |
| Node analysis in navigation mode (`NODE_ANALYZE_SKILL`) | `computer` | finding information from outside |
| Harden | `tinkering` | patching your own firewall |
| Analyze, in a fight | `infer` | reading your *opponent*, not the web |

`_intrude` is still the one attack formula — Breach and Extract share it — but it now takes the skill as a parameter, because the two are no longer the same trade. **Hack is what a Data Heist still rolls to get in** (job approaches, `LOCATION_SKILL` legwork, `REMOTE_SKILLS`); it just no longer doubles as the fighting skill.

**The deck is the weapon.** `skill_value("hack")` decides the hit; `inventory.equipped_deck_rating` decides ICE cost (`player_attack_damage` = `DECK_BASE_DAMAGE + rating`, or `BARE_JACK_DAMAGE` bare). No new `Item` field/`Slot` — a cyberdeck is already a `slot=None` item, and a deck's rating is its Logic bonus, so the four existing decks work unchanged. (Any non-deck `slot=None` `Item` would now be read as a deck — a load-bearing convention.)
...
**`CyberdeckScreen` (`screens/info_screens.py`) is where a deck's programs get installed/uninstalled** — its own sidebar category + `d` binding, split from `InventoryScreen` because `active_deck_entry` is a cyberdeck-specific question. **A tight sidebar row budget fails silently**: `CorpMapScreen`'s `#categories` fit only 9 rows at 80×24 and clipped the 10th without error. Drive the real screen at `size=(80, 24)` and check container vs. virtual size.
...
**Surfaced as a Surveillance Log panel** — a collapsed-by-default `Collapsible` in `CorpScreen`/`CorpMapScreen` (read-only history, unlike Academy/Research Facility). Each row: `"Day {day} — {who} spotted in {territory}"`, resolved at display time. `app.notify()`s once/day with just a count, not one toast per sighting.
...
- **Runner** — `BuildSelectScreen` (preset or hand-built)→`CharacterCreationScreen`→`CorpMapScreen`, with `CorpScreen` reachable as one activity among many (the map's own "Corp" category, below).
...
**`CorpMapScreen` is the home screen for both** (`load_state` always reopens it, part of the save bundle) — it carries its own left-side category sidebar (`#categories`) alongside the map itself, no separate menu screen one level down any more. Selecting gigs/jobs/legwork/Local (runner) or the corp's own action list (`_RUNNER_CATEGORIES`/`_CORP_CATEGORIES`, picked by `corp_only`) renders inline in the main panel, replacing the map; `escape` (`CorpMapScreen.action_back`) returns to the map. Phone/Gear/Cyberdeck/Skills/Research Tree still push their own screen on top of the map, same as before. The `Character` still exists in a corp-only run (carries `elapsed_hours`/`day`) — just never built or played.
...
## Corp map (`shadowguy/corpmap.py`, `shadowguy/corpmap_gen.py`, `shadowguy/factions.py`)

**Two modules, split at "read a map" vs. "lay one out".** `corpmap.py` is the model every other system touches — `LocationKind`/`Location`/`LocalCharacter`/`Territory`/`CorpMap`, the per-kind world tables (`LOCATION_SKILL`, `location_stat`), the modifier levers and the two owner profiles, lodging/safehouse pricing, `has_home`/`claim_territory`/`expansion_candidates`/`territory_distance`, and `render_ascii_map`. `corpmap_gen.py` is generation only: the grid, the blob (`_grow_region`), the wiring (`_connect`), the bloc race (`_grow_blocs`), `_player_start`, gang scatter (`_place_gangs`), the out-of-band injections (`_plan_injections`), every naming table, and `generate_corp_map` itself. **Every bare `_name` in the rest of this section lives in `corpmap_gen.py`** unless it's one of the modifier functions. Fifteen modules import `corpmap`; only `app.py` (and the test fixtures) import `corpmap_gen`, which is the whole point — one map is generated per run, so 650 lines of layout shouldn't ride along everywhere a `Territory` is read. The arrow is one-way: `corpmap_gen` imports `corpmap`, never the reverse.

The **modifier cluster stays on the model side** (`_clamp`/`_development`/`_corp_modifiers`/`_neutral_modifiers`/`make_modifiers`) even though only generation calls `make_modifiers`: `claim_territory` is runtime gameplay and reseeds a district through `_corp_modifiers` exactly the way generation does, so these are the rules for what a district's levers *are*, not for where districts go. `make_modifiers` is public (not `_make_modifiers`) precisely because it's the one name that crosses the split.
...
The scouting skill per location kind lives in `corpmap.LOCATION_SKILL` (flavor separately in `jobs.LEGWORK_APPROACH_TEXT`) — leans on perception/agility, logic on wired places, cool where the read comes from conversation.
...
**`CorpMapScreen`'s row budget no longer fits 80×24** — at `TERRITORY_COUNT` 65 the board itself is 15 lines tall (was 11 at 38 nodes), taller than `#map_scroll`'s ~12-row share of an 80×24 screen once `#territory_summary` takes its rows, so the map now needs a few rows of *vertical* scroll too, not just horizontal. `#territory_summary` (the hovered/selected territory's name/owner/value/gang/fixer, borders, modifier levels, and travel hint) is a **fixed `height: 4`**, not `auto` — deliberately, so switching territories never resizes the panel and shoves `#map_scroll`'s scroll position around; a line that would otherwise wrap past 80 cols just clips instead of growing the box. Always drive the real screen at `size=(80, 24)` and compare `#map_scroll`'s `content_size.height` to the map's line count, not panels in isolation.
...
## Corporate HQs & officers (`shadowguy/corpmap_gen.py`, `shadowguy/factions.py`, `screens/shop_screens.py`'s `CorpHQScreen`)
...
## Street gangs (`shadowguy/gangs.py`, `shadowguy/corpmap.py`, `shadowguy/corpmap_gen.py`)
...
**Turf is scattered, not grown.** `corpmap_gen._place_gangs` hands each gang `GANG_TURF_MIN..MAX` (2–3) territories from one shuffled pool of unclaimed, non-start ids — no contiguity requirement, no two gangs ever share a territory (free via the shared pool). Lands on `Territory.gang_id`, deliberately separate from `owner` (a gang's ground stays `owner == "neutral"`). `generate_corp_map` guards feasibility up front: `len(GANGS) * GANG_TURF_MAX` must fit the unclaimed non-start pool.
...
**Staffing is presence only, not a ladder.** `_make_gang_members` gives one `LocalCharacter` per `GANG_RANKS` tier (same helper `_make_officers` uses) but no rep/standing gate or dialogue. Soldier/lieutenant appear in `CorpMap.characters()` but never surface in Locals (nothing moves their `local_standing`). `GANG_DEN` *is* routed through `CorpMapScreen`'s Locals panel (`GangDenScreen`) — see Gang deliveries below — just not to a dialogue screen the way `CORP_HQ` is.
...
The one gang-given job, and deliberately not a `Scene` at all — it was a `JobArchetype` first, but a gang doesn't own territory the way `generate_job`'s employer-selection assumes, and the job itself is about *reaching a place in time*, not passing checks along the way. So it's tracked directly: `Character.smuggling_job: SmugglingJob | None`, one at a time.

- **Picked up in person.** Visiting a `GANG_DEN` opens `GangDenScreen` (`CorpMapScreen`'s Local-tab routing, same pattern as `JunkyardScreen`/`HospitalScreen`). If the runner has no active `smuggling_job`, "Take a delivery job" calls `generate_smuggling_job(gang_id, character.location_id, ...)` — the den's own territory *is* the pickup, no separate roll for it. The destination is any other territory on the map regardless of owner; the deadline is `SMUGGLING_BASE_DEADLINE_DAYS` (2) + `SMUGGLING_DEADLINE_DAYS_PER_HOP` (1) × `corpmap.territory_distance` (a BFS hop count over `Territory.connections`) — a cross-map run gets more time than a next-door one. Reward reuses `REWARD_BASE` at the current day's tier, the same table jobs pay from.
- **Delivered in person.** `CorpMapScreen` shows a "Deliver the package" item whenever `smuggling_job` is set (same always-visible pattern as "Rest"), previewing a travel hint if the runner isn't at `destination_territory_id` yet. Clicking it off-site does nothing; on-site it pays `reward_cash`, raises gang standing by `GANG_JOB_STANDING_GAIN` (2), and clears the job.
- **Missing the deadline** is checked once per day tick (`app._apply_day_tick`, alongside crew wages/job expiry): once `day > deadline_day`, the job clears itself and gang standing drops by the same `GANG_JOB_STANDING_GAIN` instead of rising — the mirror of a completed delivery, not a separate constant.
- **First-slice numbers, not balance-simulated** — the deadline formula and reward reuse are easy to retune once there's a feel for how tight cross-map deliveries actually play.

## Junkyards (`shadowguy/corpmap.py`, `shadowguy/corpmap_gen.py`, `shadowguy/shops.py`, `screens/shop_screens.py`'s `JunkyardScreen`)
...
**The one action is `shops.scavenge`**, wired through `JunkyardScreen` (`CorpMapScreen`'s Local-tab routing, alongside `HospitalScreen`). Rolled against Tinkering at `SCAVENGE_DIFFICULTY` (11, legwork's `NEARBY_DIFFICULTY` tier) — an eye for what's actually usable versus rust. A made check adds one random `SCAVENGE_MATERIALS` entry to inventory; a critical adds `SCAVENGE_CRITICAL_FINDS` (2) distinct ones; a miss adds nothing. Costs `SCAVENGE_HOURS_COST` (4) — a gig-length activity, not a full day like a hospital stay — spent either way, win or miss, same as a gig or legwork.

**`SCAVENGE_MATERIALS`** (Armor Plating, Rubber, Salvaged Optics, Wire, Screws) are `shops.LOOT_ITEMS` entries with bare `{}` bonuses — no equip effect of their own. Sellable through the ordinary Pawn Shop `sell_item` flow (any `Item` can be), but the real sink is the workshop (see Rest, lodging & property): `install_mod` and `craft_consumable` both consume them by id. `LOOT_ITEMS` also holds `STOLEN_DATASHARD_ID` (matrix.py's unrelated CACHE-node reward) — `SCAVENGE_MATERIALS` is a narrower tuple so a Junkyard can't turn up a datashard, nor a Cache node scavenged material.
...
## Shops (`shadowguy/shops.py`, `shadowguy/inventory.py`)

`shops.py` owns what exists, what it costs, and buying/selling it; `inventory.py` owns
everything downstream of a purchase — equip state, deck programs, popping a
consumable — so combat/matrix/tactical, which only ever care about current equip
state, don't have to pull in the whole retail catalog to get it (`inventory.py`
imports `shops.py`, never the other way).
...
Selecting one pushes a `ShopScreen`. `CATALOG` maps shop kind → `Item`s. Buying spends cash, appends an `InventoryItem(item_id, equipped=True)` to `Character.inventory` (a list, duplicates allowed). Items are persistent, not consumable; only an equipped item counts (`inventory.equipped_bonus` skips unequipped).

An `Item` carries more than a flat bonus: `bonuses` (dict[stat,int], folded into `stat()`), `skill_bonuses` (per-skill, via `inventory.equipped_skill_bonus`), `slot`/`defense`/`concealment`/`two_handed`, `skill`+`damage` (weapon profile, below), `travel_reduction` (vehicle slot), `min_standing` (stock gate), `recharge_rounds`/`stun_damage` (combat pacing/knockout).
...
**A `Slot.WEAPON` item carries its combat profile** (`Item.skill`+`damage`) — the only place it's written, checked both ways at import (a weapon needs a profile; a profile needs a weapon slot). The catalog covers every weapon skill except `gunnery` — clubs (knuckles), blades (knife, monoblade, katana), pistols (pipe pistol, both tasers), automatics (machine pistol, assault rifle), longarms (pump shotgun), archery (compound bow), throwing (throwing knives) — so no build is stuck with a weapon it can't use. `gunnery` has nothing because there is no turret to mount. Damage stays in a 4–7 band across all of them, with the 7s behind the standing-2 gate. What separates them is reach (below), whether the weapon costs both weapon slots, and `recharge_rounds` — so the ranged ladder runs cheap-and-slow (bow, 550eb, fires every other turn) → same damage every turn (shotgun, 650eb) → same damage with a hand free (machine pistol, 800eb). `concealment` is set on every weapon but read by nothing outside the import-time bounds check, so it is not a balance lever today. **Hand-set against the existing curve, not balance-simulated.**

**`recharge_rounds` is only meaningful at 2+.** A 1 is set when the weapon fires and ticked back off before the player's next action, on both surfaces — so it does nothing at all. `taser_h6` carries a dead 1 (its "fires every 2 rounds" comment is aspirational); every weapon added since uses 2. Fixing the off-by-one would change taser pacing, so it hasn't been.

**`COMBAT_ONLY_EFFECTS`** (the three grenades: damage-all, stun, clean escape) can only be reached in a fight — refused by `inventory.use_consumable` outside one, without being spent.
...
## Cyberware (`shadowguy/cybernetics.py`, `screens/shop_screens.py`'s `RipperdocScreen`)
...
**Four quality grades, generated rather than hand-authored per grade**: `CYBERWARE_TIER_MULTIPLIERS` maps a grade to `(price_mult, humanity_mult)`, both relative to the same piece's **Deltaware** row (the baseline catalog) — Trashware (secondhand): -50% price/+100% humanity; Betaware (lightly tailored): +20% price/-20% humanity; Alphaware (bespoke): +100% price/-50% humanity. A grade changes only cost, never effect — enforced by generating the other three via `dataclasses.replace(deltaware_row, ...)`. `Cyberware.tier` validated against `CYBERWARE_TIER_IDS`; ids follow `{base_id}_{tier}` (e.g. `smartlink_alphaware`).

**Acquisition is a `CYBER_CLINIC` and `RipperdocScreen`.** This section used to say the wiring was deferred because "adding a shop kind touches corpmap's name pools, `LOCATION_SKILL`, `GENERATED_KINDS`, legwork text, and gig templates all at once" — that was wrong by the time it mattered. `LocationKind.CYBER_CLINIC` had **already** been a fully generated kind for a long time (name pool, owner roles, `LOCATION_SKILL` → `infer`, gig template, `LEGWORK_APPROACH_TEXT`, `BuildingKind.OFFICE`, and the `FactionSpecialty.CYBERNETICS` anchor), so clinics stood on the map and could already be cased or robbed. The only missing pieces were a screen and one `_push_location_screen` branch.

A clinic is deliberately **not** in `SHOP_KINDS`: that tuple routes to `ShopScreen`, which sells `Item`/`Consumable`/`Program`, and cyberware is none of those. It gets its own branch and its own screen, the same way `HOSPITAL`/`JUNKYARD`/`GANG_DEN` do. `CorpMapScreen`'s local-box stock preview does special-case it, since the preview is just "names, filtered by standing, truncated" and that shape is identical.

**Standing gates which grade a doc will fit, and the gate runs opposite to price.** `Cyberware.min_standing` is the same field `shops.Item`/`Consumable`/`Program` each carry, and `RipperdocScreen` hides a row above it outright rather than showing it locked — exactly `ShopScreen`'s `if item.min_standing > standing: continue`. It's set per *grade* (`CYBERWARE_TIER_MIN_STANDING`: Deltaware 0, Trashware 0, Betaware 1, Alphaware 3), never per piece, because the grade already says what it means: Trashware is the cut-price secondhand knockoff that costs more of you, so any back-alley grafter fits one to a stranger, while Alphaware's bespoke tailoring is what a doc saves for regulars. **Deltaware is open to everyone**, so no *effect* in the catalog is ever locked behind a relationship — standing buys a better trade-off on the same implant, never a bonus you couldn't otherwise get.

**No standing discount**, unlike `shops.buy_price`: standing decides what a doc is willing to cut into you, not what they charge. That also keeps `cybernetics.py` a leaf — a discount would mean importing `shops`.

**Standing is floored at 0 for gating** (`_effective_standing`). Every row's `min_standing` is `>= 0`, so a *negative* standing — one failed gig at the clinic is enough, `GIG_FAIL_STANDING_HIT` — would otherwise hide the whole catalog rather than just the gated grades, breaking "Deltaware is open to everyone" outright. A doc you've annoyed stops cutting you deals on the good chrome; they still sell you the baseline. **`shops.py` deliberately does not do this** — a shopkeeper who dislikes you can refuse their entire stock, because no shop catalog commits to an always-available grade the way this one does.

**Whose standing gates the shelf is the *best* of the clinic's people** (`RipperdocScreen._contact`), not `characters[0]`. `ShopScreen` can assume the first character is "the owner" because every `SHOP_KINDS` location is generated with exactly one; a clinic isn't in `SHOP_KINDS`, so `_make_characters` gives it 1–2 like any other non-shop kind, and `refresh_gigs` assigns a gig to a *random* one of them. Reading only the first would mean roughly half a clinic's gigs moved a standing the shelf never looked at. The header names whoever the current best contact is.

**Every catalog row must be installable within `HUMANITY_BASELINE`**, guarded at import in `character.py` (the module that sees both the constant and the catalog). The tier multipliers make this easy to break from the Deltaware side: `adamantium_bones` at a `humanity_cost` of 3.5 would put its Trashware knockoff at 7.0, past the baseline of 6, so that row could never be installed by anyone — invisible while nothing sold cyberware, a permanent dead shelf line once something did. Its base cost is 2.8 for exactly that reason (same for `reflex_coprocessor`/`grapple_rig_cyberarm`, trimmed from 3 to 2.5 when Trashware's 2x replaced the old Tier 4's 1.6x). The guard used to only ask that each `CyberSlot` held *some* affordable piece, which is how one bad row hid in a slot that had others.
