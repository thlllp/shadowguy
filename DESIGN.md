...
**Nothing rolls a core stat directly.** Each stat carries skills (`skills.SKILLS`, **30** total); a `Choice` names a *skill*, and `skills.skill_value()` is what the dice see: the skill's tied stat (gear included) plus the rank invested in that specific skill. **Every weapon skill is an agility skill** — `Short Blade`/`Long Blade`/`Blunt` (once strength's) and `Firearms`/`Misc Weapons` (once perception's) all sit on `agility`, on the reading that what a weapon rolls is handling it, not the muscle behind it or the eye down the sight. That leaves the spread lopsided on purpose: agility **10**, perception **4** (its base, the weapon pair having left), strength **2** (`Grapple`, `Lift` — unarmed and hauling, which are not weapons). Strength is now a narrow stat that buys the bare-hands attack and the force-it job approaches and little else, and agility is the widest stat in the game: it already carried `Dodge` (the combat exit and the defense roll) and `Stealth`, and now carries every way to swing or shoot as well, so an agility build is both the hardest to hit and the best-armed. **Nothing in the balance simulation has been re-run against this**, and the old sim figures below (which measured a strength/agility split) predate it. Body stayed at four, net: `Resist Poison`/`Resist Disease` folded into a single `Fortitude` (both were "the body shrugging off an internal assault" wearing two names), `Lung Capacity` was cut outright, and a new `Running` skill took over the two job approaches (both literally about outrunning pursuit) that `Lung Capacity` used to cover. Perception is down one from its old five-skill base for the same reason as Body's `Fortitude` merge: `Read Face`/`Read the Room` folded into a single `Intuition` (both were "reading a person or a room" wearing two names). Nothing enforces five-per-stat, and cost is per-skill anyway, so extras and shortfalls just make a stat broader or narrower, not stronger or weaker. `Character.skill_ranks` is `dict[skill_id, int]`, fully populated (every skill starts at `STARTING_SKILL_RANK`). A skill id absent from `SKILLS_BY_ID` raises from `skills.skill_for()`, the single chokepoint — `Scene.__post_init__` runs it over every choice, so a typo fails when the scene is *built*, not mid-roll.
...
**Not every skill is rolled by something.** Coverage: 26 distinct skills across `jobs.ARCHETYPES`' approaches, 8 via `corpmap.LOCATION_SKILL` (legwork), the per-kind approaches in `gigs._GIG_TEMPLATES`, and combat (`abstract_combat.available_actions`: brace/read-the-fight/face-them-down/break-and-run, plus whatever skill each equipped weapon rolls). `Leadership` (`cool`) is *read* rather than rolled — scales crew recruiting terms (see Runners & crew). Gaps: `Blunt`, `Long Blade` and `Misc Weapons` are weapon-gated (need to own the weapon; `Firearms` is the one weapon skill a job approach still reaches, on Wetwork's objective); `Sight` is reached only via legwork at a `WEAPON_SHOP`. **`Blunt` left the job pools with the weapon-skills-to-agility move** — a stage's approaches must sit on three different core stats (guarded at import), and every stage that offered a blunt/short-blade way through already offered an agility one, so those approaches became the strength skill their flavor already described (`Lift` forces the vault/case/rack, `Grapple` puts down whoever's in the way) and one matrix beat became `Toughness`. The stat spread across approaches survived it — agility 23, cool 19, logic 19, body 16, strength 16, perception 12 — because the replacements are strength skills. The three presets buy narrow on purpose — a preset should read as an archetype, not a hedge — which is what makes each bleed on unsuited job stages.
...
- The begin-gate can't deadlock: ranks 2–4 always cost 1, and 20 points can never push enough of 30 skills past rank 4 to exhaust the 1-point buys.
