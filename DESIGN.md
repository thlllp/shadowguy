...
**Both attacks and grenades aim through the same free-look reticle**, a small modal turn state (`TacticalState.aim_cursor`/`pending_grenade_index`): arrow keys drive the cursor instead of the player while `aim_cursor` is set (`move_aim_cursor`, unbounded except by the grid edge — legality is checked only at confirm, not while moving), and `cancel_aim` backs out for free either way. `pending_grenade_index` is what tells the two aims apart (`None` means an attack is aiming). Pressing fire (`f`) always opens `begin_attack_aim` rather than resolving instantly — it seeds the cursor on `best_shot`'s pick (or the player's own tile with no shot in range) and `confirm_attack_aim` fires on whatever `legal_attack_target` finds standing there, using the reaching weapon that does the most damage (`_best_weapon_against`). Grenades work the same way one level more specific: `throw_grenade` reuses `combat._throw`'s three `shops.EffectKind` branches (`combat_consumables`, promoted public for exactly this reuse), but `COMBAT_STUN`/`COMBAT_DAMAGE_ALL` land as a blast — every enemy within `GRENADE_RADIUS` (chebyshev distance 1, a literal 3x3 square) of a target tile, not everyone standing, since the grid gives a grenade something abstract combat doesn't have: a place to actually miss. `COMBAT_ESCAPE` (a smoke grenade) stays positionless, same as the abstract fight — walking out isn't aimed at anyone. `begin_grenade_aim`/`confirm_grenade_aim` are the grenade-specific halves of the same flow, gated by `legal_grenade_target` (`GRENADE_RANGE` + `has_line_of_sight`, same shape as a weapon's range+LOS gate). Backing out of either aim, or leaving one unresolved, costs nothing — only a resolved shot or throw spends the turn's action (or, for a grenade, pops the consumable). First-slice tuning, like the grenade catalog itself (`shops.py`'s `_CONSUMABLE_ROWS` comment) — `GRENADE_RANGE`/`GRENADE_RADIUS` aren't yet balance-simulated.
