...
**Tiles outside the player's current FOV render dimmed, not hidden.** There's still no fog of war — the whole map is always drawn (`TacticalScreen`'s own precedent) — so dimming says only "you can't presently see this," keeping layout and cover readable while making LOS legible without a separate indicator. `tactical.visible_tiles` exists for this: it hands back the whole `[y, x]` FOV bool array in one call. A renderer must **not** call `has_line_of_sight` per tile — that recomputes the FOV from scratch for every cell on the map.
