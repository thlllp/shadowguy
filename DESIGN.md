...
The CorpMapScreen **Local** category renders one Collapsible box per the runner's node's `Location`s, plus a Fixers box if any are present (`#local_boxes`, a phone-tile-style grid, at most one box open at a time — `CorpMapScreen.on_collapsible_expanded` collapses every other box in the same grid when one opens). A box's preview covers who works there (`Location.characters`), any gig offered there (`app.location_gigs`), other runners present (bar `Location`s only — the one place `rivals.py`'s DRINKING activity ever puts a rival runner), and a shop's stock (names only, capped at `_STOCK_PREVIEW_COUNT` before "+N more" — pricing lives on `ShopScreen`, not here; `WEAPON_SHOP` alone stocks 20+ items). An "Enter" row inside the box is what actually pushes the location's own screen: the five shop kinds → `ShopScreen`, `BAR` → `BarScreen`, `CORP_HQ` → `CorpHQScreen`, `HOSPITAL` → `HospitalScreen`, `JUNKYARD` → `JunkyardScreen`, `GANG_DEN` → `GangDenScreen`, `REAL_ESTATE` → `RealEstateScreen`, `APARTMENT`/`SAFEHOUSE` → `SafehouseScreen`. The rest are display-only. `CORP_HQ`/`GANG_DEN` are the two that need their owner threaded in as well as the `Location` (`FACTIONS_BY_ID[here.owner]` / `GANGS_BY_ID[here.gang_id]`).

**The map's own hover/selection preview (`#map_local_boxes`, shown in map mode below `#territory_summary`) is the same box design**, built by `_location_box`/`_fixers_box` with `surface="map"` instead of `"local"` (only the id prefixes differ, to keep the two grids from colliding when both happen to be mounted at once). The one behavioral difference: a map-hover box can't be *opened* unless the runner is actually standing in that territory — `on_collapsible_expanded` re-collapses it and notifies "Travel to `<name>` first." otherwise, the same message `map_local_`'s own Enter-row handler already gives. It's a preview of what's on the map, not a place to transact from a distance. Both box grids sit in a bounded, independently-scrolling `height: 14` container (`#local_boxes_scroll`/`#map_local_boxes_scroll`) rather than `1fr` — they're a strip below the map/activity list, not the main event, and an expanded box's content (a long stock list) can still run taller than that and needs to scroll internally instead of pushing everything else off-screen.
